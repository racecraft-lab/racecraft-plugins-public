...
| Python runner | Generated Claude and Codex payloads include `speckit_pro_runner/`, and installed runtime gates invoke it with Python 3.11+ argv calls. | [Install: Claude Code](/racecraft-plugins-public/install/claude-code/#installed-runtime-prerequisites), [Install: Codex](/racecraft-plugins-public/install/codex/#installed-runtime-prerequisites) | Runner manifests and checksums are completeness and local integrity evidence, not signing, SLSA, provenance, or a cryptographic trust-chain guarantee. |
...
| Treat runner checksum checks as bounded completeness checks. | They help detect missing, stale, or mismatched generated payload files without promising a signed supply chain. | [Update & Rollback](/racecraft-plugins-public/update-and-rollback/) |
