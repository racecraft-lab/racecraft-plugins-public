...
| Stale payload | Both | Inspect generated payload references for Claude Code and Codex, and compare them with authoring source. | Side effect: generated or copied payload files may change. Regenerate or copy payloads through the source-owned payload-completeness runner flow, then refresh the platform install. | Payload files align with checked-in source. | Reload or restart after platform install state changes. | [source vs dist](/racecraft-plugins-public/reference/source-vs-dist/), [contribute and release](/racecraft-plugins-public/contribute-and-release/) |
