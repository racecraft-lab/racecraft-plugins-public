...
- **`AUTH-2`** — Documented authentication modes, in the page's precedence order: `CLAUDE_CODE_USE_BEDROCK` / `CLAUDE_CODE_USE_VERTEX` / `CLAUDE_CODE_USE_FOUNDRY` (cloud providers), `ANTHROPIC_AUTH_TOKEN` (sent as `Authorization: Bearer`), `ANTHROPIC_API_KEY` (sent as `X-Api-Key`), `apiKeyHelper`, `CLAUDE_CODE_OAUTH_TOKEN` (a long-lived token from `claude setup-token`), and interactive subscription login (`/login`). `[FACT]`
  ...
  - Note: the shipped agents configure no per-agent authentication (repository fact), so the CAR-002/003 evaluation environment runs them under the session's auth mode (any of `AUTH-2`'s documented modes); the manifest `required_capabilities.client` is therefore "Claude Code, any supported auth mode." `[POLICY]` — proposed SpecKit Pro evaluation-environment policy: `AUTH-2` enumerates the modes, but subagent auth *inheritance* is not separately documented, so running the agents under the session mode is a recorded project decision, not a cited fact.
...
|----|---------------------|--------|
| **`CAP-Q1`** | Does the `opus` Claude Code alias resolve to `claude-opus-4-8` in the pinned benchmark environment, and is that alias→dated-ID binding stable given "latest"-family resolution, allowlist pinning, and `ANTHROPIC_DEFAULT_OPUS_MODEL` re-pointing (`ALS-opus`, `RES-3`, `ALS-repoint`)? | CAR-002 probe of the `opus` route's environment-time resolved ID (plus the resolved model's effort-acceptance, EFF-1); gates candidate-tuple binding for the five `opus` executor agents (analyze/checklist/clarify/implement/phase) and the `opus` upshift candidate on the six `sonnet` analyst/validator/author agents. |
| **`CAP-Q2`** | Does the `sonnet` alias resolve to `claude-sonnet-5` in the pinned environment, and is that binding stable under the same floating-resolution/allowlist/re-point risks (`ALS-sonnet`)? | CAR-002 probe of the `sonnet` route's resolved ID (plus the resolved model's effort-acceptance, EFF-1); gates candidate-tuple binding for the six `sonnet` analyst/validator/author agents (codebase/consensus/domain/gate/spec-context/uat), the `sonnet` candidate on the five executors, and the helper's `sonnet` latency-oriented alternative. |
...
- **`autopilot-fast-helper`** — text-only, no tools; its qualification telemetry is the same mandatory set
  (the `-p` result `result`/cost, plus `resolvedModel` from the SubagentStart hook / Agent-tool response —
  a companion surface, not the `-p` JSON result) against the `haiku`/low candidate (`CAP-Q3`); no
