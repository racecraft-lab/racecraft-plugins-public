...
| PRD/design/roadmap mentions | `docs/prd-tool-agnostic-capability-discovery.md:16-28`, `docs/ai/specs/tool-agnostic-capability-discovery-design-concept.md:20-27` | historical/planning provenance | allow | Keep as provenance unless TACD-004 finds active guidance masquerading as history. |
...
|---------|------------|----------------|------------|------------------------|-------------------------------|
| Claude Code | Installed tools | source-backed plus environment-specific | medium | Plugin agents declare concrete tools in frontmatter, including MCP and built-in fallback tools. Example: `speckit-pro/agents/codebase-analyst.md:12-20`. Plugin limitation docs say agents inherit parent-session MCP availability, not their own MCP server declarations: `speckit-pro/skills/speckit-autopilot/references/plugin-limitations.md:30-38`. | TACD-002 should direct Claude agents to choose by needed capability and report the chosen path; missing MCPs should fall back to built-in tools with lower confidence. |
| Claude Code | MCP/app connectors | environment-specific | medium | Plugin agents can list MCP tools, but plugin agents cannot declare their own MCP server connections; parent session configuration decides availability. Evidence: `speckit-pro/skills/speckit-autopilot/references/plugin-limitations.md:13-15` and `speckit-pro/skills/speckit-autopilot/references/plugin-limitations.md:30-38`. | TACD-003 should keep a non-blocking advisory; TACD-004 should test behavior without assuming a fixed connector list. |
| Claude Code | Skills/plugins | source-backed | high | Claude plugin manifest identifies `speckit-pro` as the plugin package: `speckit-pro/.claude-plugin/plugin.json:1-20`. The repo has skill and agent roots under `speckit-pro/`. | TACD-002 can use a shared reference if active Claude agent/skill entrypoints point to it or carry an approved equivalent. |
| Claude Code | Repo-local helpers | source-backed | high | Agents already name built-in local fallbacks such as `Read`, `Glob`, and `Grep`; example fallback chain is in `speckit-pro/agents/codebase-analyst.md:57-69`. | If no installed context tool exists, local file tools remain valid fallback. Output should include confidence and local file references. |
| Codex | Installed tools | source-backed plus environment-specific | medium | Codex TOML agents encode capability instructions in `developer_instructions`; example codebase discovery preferences and fallbacks are in `speckit-pro/codex-agents/codebase-analyst.toml:35-52`. Actual callable tools depend on the active Codex session and installed connectors. | TACD-002 should use capability names and Codex tool-discovery surfaces, not hardcoded vendor-first wording. |
| Codex | MCP/app connectors | environment-specific | medium | Codex dependency metadata names MCP dependencies for the autopilot skill sidecar: `speckit-pro/codex-skills/speckit-autopilot/agents/openai.yaml:9-16`. Runtime availability still depends on installed connectors. | Exact dependency IDs should remain metadata until TACD-002 proves a generic declaration path or approved equivalent. |
| Codex | Skills/plugins | source-backed | high | The Codex plugin manifest points skills to `./codex-skills/` and describes bundled Codex custom-agent templates: `speckit-pro/.codex-plugin/plugin.json:22-43`. | Shared reference can work only if Codex skill entrypoints and installed custom-agent templates point to it or carry an approved equivalent. |
| Codex | Repo-local helpers | source-backed | high | Codex agent instructions explicitly include fallbacks to search/read files when RepoPrompt-like tools are unavailable: `speckit-pro/codex-agents/codebase-analyst.toml:40-52`. | If no installed connector exists, Codex can continue with local search/read/shell helpers and lower confidence where evidence quality drops. |
...
| Named optional-tool matches | `rg -l "...named terms..." speckit-pro tests/speckit-pro docs -S | wc -l` | 59 files | TACD-004 should use category-aware enforcement rather than a blind ban. |
| Capability-language matches | `rg -l "...capability terms..." speckit-pro tests/speckit-pro docs -S | wc -l` | 84 files | Existing repo already has capability vocabulary, but it is not a final directive contract. |
