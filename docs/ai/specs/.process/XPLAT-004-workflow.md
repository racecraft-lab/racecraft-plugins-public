...
| Reviewability Diff Gate | Pass with exception | Final diff is size/file-count blocked but accepted through branch-added `Reviewability-Exception: infra`; blocked operations are empty | Evidence: `specs/xplat-004-cross-platform-runner-foundation/.process/final-reviewability-state.json` |
