...
| 4 | Gap | EH-CHK010/011: does `fallback_loop` fire pre-walk like the other three structural rejections? | [spec] | 1 | high-confidence | **Keep the partition — three pre-walk, `fallback_loop` in-walk.** A loop is *syntactically* visible in the declared policy but not *semantically equivalent* to FR-020's predicate, and the two readings produce **different pinned bytes for the same policy**: FR-004's first-match walk means a duplicate later in the chain is never reached if an earlier route resolves, so pre-walk detection would convert a *resolving* policy into a failing one; and the fan-out cap makes a duplicate beyond the cap provably unreachable. Under pre-walk suppression FR-020's "terminate without repeating" becomes vacuous — no pinned byte could distinguish conformance, fatal in a byte-compared corpus. Four things also depend on dynamic detection, including FR-001's one-module justification, which would be orphaned. **The tension resolves against the domain-focus text, not the spec** — the roadmap imposes no timing at all, so nothing settled was overturned | spec-context-analyst |
| 4b | Gap | EH: `exhausted_budget` with `minItems: 1` is unsatisfiable in two reachable cases | [spec] | 1 | high-confidence | **Independent defect, surfaced by item 4 but not caused by it.** A pre-walk rejection fixes all three counters at 0 while two caps carry `minimum: 1`, so its at-cap set is *necessarily* empty; and a `no_safe_route` from an empty fallback list can end below every cap. Fixed: required only when ≥1 class is at cap, **omitted — not empty** — otherwise, which preserves the "presence means at-cap" reading. Rejected the `minItems: 0` alternative because `[]` would destroy that semantics | spec-context-analyst |
