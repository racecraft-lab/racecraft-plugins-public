...
| Skill and reference prose — markdown | 7 | 530 |
| Authored tests | 6 | 1131 |
| Generated payloads, manifests, cache proofs — not human-reviewed | 62 | 3040 |
| Specification and planning documents | 19 | 4938 |
| **Human-reviewable total (logic + prose + authored tests)** | **17** | **1999** |

**Correction (2026-08-05).** An earlier revision of this section reported 29
files / 2020 lines and said the result exceeded *both* block thresholds. That was
wrong on the file dimension: twelve `installed-cache-proof*.json` fixtures were
counted as authored tests when they are regenerated by
`scripts/refresh-release-artifacts.py`, as are the runner manifest and `.sha256`.
The corrected file count is **17**, which is **warn** against 15 warn / 25 block —
not a block — and is *exactly* the figure `plan.md` declared. Only the line count
exceeds its block threshold.
...
400 warn / 800 block and 15 warn / 25 block. The line count exceeds its block
threshold; the file count does not, and matched the declaration exactly.
...
guard (69), and small registry and schema additions. Second, 1131 of the
remaining lines are authored tests, and **992 of those are a single table-driven
fixture file** — half the entire human-reviewable diff in one file whose rows
review far faster per line than logic does — while 530 are skill and reference
...
The estimator's forward guess was 459; the outcome is 1999 human-reviewable
lines of which 338 are logic and 992 are one fixture table. See
`ART-006-retrospective.md` for what that gap does and does not say about the
estimator — re-run with the final signals it lands far closer than the raw
comparison suggests, and the residual is dominated by a single file whose size no
input signal can express.
