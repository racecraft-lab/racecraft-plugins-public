...
| Tasks | `/speckit-tasks` | ✅ Complete | 24 tasks (T001–T024) in 5 groups; all 24 FRs mapped; 10 [P]; RED-before-GREEN; Codex mirror paired (T007↔T008). +1 file beyond plan: tests/unit/test-moc-lint-exit-codes.sh (subprocess driver for the 3-way exit contract). Reviewability gate: pass (transition exception). |
