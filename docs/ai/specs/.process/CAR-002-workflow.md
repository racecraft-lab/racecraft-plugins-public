...
| Implement | `/speckit-implement` | ✅ Complete | All 40 tasks; TDD RED→GREEN throughout; operator live probe run (T015) captured `CAR-002-RCS-2026-07-17-V3`; **G7 pass — full default suite 3193/3193** (L1 1428, L4 1579, L5 186), zero live calls, payload boundary clean |
...
| 2 - WP1 probe tool + snapshot | T009–T018 | 10/10 ✅ | Pure logic + FR-003 controls + live boundary + capability capture + subagent mechanism; operator run captured `CAR-002-RCS-2026-07-17-V3`; **sanitizer broadened to redact session UUIDs (privacy-scan defect caught + fixed, re-probed clean at V2)**; committed-snapshot validation + docs-surface guard |
...
- **Headline evidence**: CAP-Q5: both dispatch surfaces hard-reject an unavailable model — `-p --model <unavailable-id>` rejects directly, and subagent-frontmatter dispatch fails at spawn time with a terminal model-access error (no fallback), with the full FR-010 unset-proof recorded on both (bare-platform behavior, not config masking). Note: an independent post-implementation code review corrected an earlier V2 mis-derivation that read the parent session's model as a subagent soft-remap; V3 records the accurate hard-rejection on both surfaces.
