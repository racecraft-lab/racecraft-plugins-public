...
- New Layer 4 unit test `speckit-pro/tests/unit/test-generate-uat-skeleton.sh` sourcing `tests/lib/assertions.sh`, fixtures created in `mktemp -d` with cleanup trap (mirrors `test-ensure-reviewability-preset.sh` pattern). Fixtures cover: full spec (snapshot of `specs/004-integration-verification/spec.md`), zero-stories spec (synthetic), duplicate-FR spec (synthetic), `[NEEDS CLARIFICATION]` spec (synthetic), and missing-spec error
...
- `speckit-pro/tests/unit/test-generate-uat-skeleton.sh` — New: Layer 4 unit test
