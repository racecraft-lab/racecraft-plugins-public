...
- **Prevention replaces the defeated gate.** The end-stage detective control is rebuilt
  *last*, as a backstop that triggers re-slicing — only after the automatic small path
  exists.
- **Generated PR packets are review-ready before creation.** Autopilot-generated titles
  and bodies are deterministic, neutral, and actionable for both single-PR and split-PR
  paths before any `gh pr create` call runs.
...
- **LLM-only PR authoring.** Reviewer-ready prose may be refined, but PRSG-012 is not
  an agent-only drafting feature. The first title/body packet is generated by scripts
  from spec, plan, slice, UAT, and verification evidence, then validated.
...
### 3.12 Reviewer-ready PR packet contract *(→ PRSG-012)*

- **AC-12.1**: both the single-PR and split-PR post-implementation paths generate an
  explicit PR title and PR body packet before PR creation; `gh pr create` uses
  `--title` plus `--body-file` and never relies on branch-name, commit-message, inline
  body, or manual GitHub fallback behavior.
- **AC-12.2**: the generated PR body uses neutral, reviewer-actionable sections:
  `Summary`, `What Changed`, `Why It Matters`, `How To Review`, `How To UAT`,
  `Verification`, `Scope`, and `Known Gaps`; it contains no unfilled template comments,
  stale placeholders, or patronizing labels such as `ELI5` or `Plain-English Summary`.
- **AC-12.3**: a deterministic validator blocks before PR creation when the title/body
  packet is missing, stale, invalid, placeholder-filled, or uses banned wording, and
  records the exact remediation needed in workflow evidence.
- **AC-12.4**: scripts own the first draft from spec, plan, slice packet, UAT runbook,
  and verification evidence. Human or agent refinement may only edit explicit prose
  fields while preserving generated governance sections, UAT content, and source
  markers.
...
- **Phase 7 (PRSG-012) — Reviewer-ready PR packets**: follow-on hardening for the
  generated PR title/body contract after split-PR emission and reviewer-experience
  UAT wiring exist.
...
> block PRD acceptance; PRSG-011 items were resolved in its grill-me / clarify pass.
...
### 6.1 Resolved PRSG-012 scoping decisions

The 2026-06-11 grill-me pass resolved PRSG-012's purpose:

- **Packet contract:** pre-create gate, not advisory guidance.
- **Draft ownership:** deterministic generator owns the first title/body packet; prose
  refinement is allowed only in explicit fields.
- **Coverage:** both single-PR and split-PR paths use the same contract.
- **Reviewer format:** optimize for actionable sections and neutral wording, not
  novelty labels or tutorial-style framing.
...
| Reviewer-ready PR packet contract | AC-12.* | PRSG-012 | PRSG-009, SPEC-006a/b, PRSG-010 | P1 |
...
7. Autopilot-generated PRs are review-ready at creation time: titles are explicit and
   conventional, bodies use the PRSG-012 section contract, and invalid packets block
   before `gh pr create` (AC-12.*).
