...
| Decide whether a behavior needs formal verification, choose a checker, or learn a first model | [Selective formal methods](../../skills/speckit-coach/references/formal-methods-guide.md). Assess risk and simpler checks first; record explicit selection and adapt coaching to experience. |
| Model a difficult rule in Quint, or look up Quint language, modeling, or witness guidance | [Optional Quint modeling](../../skills/speckit-coach/references/quint-guide.md). Read only the sections the model needs; SpecKit still owns selection, bounds, and gate decisions. |
...
| Design a multi-spec roadmap or split a feature | Read the complete [technical-roadmap template](../../skills/speckit-coach/templates/technical-roadmap-template.md) and [slicing heuristics](../../skills/speckit-coach/references/slicing-heuristics.md), then instantiate the template without dropping required sections. `## Progress Tracking` is mandatory: scaffold and status use it as the shared lifecycle record. Derive independently executable vertical slices with observable outcomes and explicit real dependencies; review the graph before writing. |
