...
| "install X extension", "add the X extension", "remove X extension", "disable an extension", "enable an extension", "configure an extension" | Run **Play 3 — Install / configure / remove**. Always confirm with the user before mutating. Use `specify extension add/remove/enable/disable/set-priority`. After install, point at `.specify/extensions/<id>/<id>-config.yml` (shared) + `<id>-config.local.yml` (personal). If the extension should fire automatically, register a hook in `.specify/extensions.yml` (the autopilot picks this up dynamically). |
