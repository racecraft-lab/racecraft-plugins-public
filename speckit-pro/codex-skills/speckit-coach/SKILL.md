...
| **Plugin Usage** | |
| "install codex agents", "subagents missing", "repair codex install", "refresh codex agents" | Explain the current Codex model: `speckit-autopilot` and `speckit-coach` ship skill-local `agents/openai.yaml` metadata sidecars, not installable custom-agent manifests. The default path uses built-in `worker` / `explorer` / `default` agents and does not require a separate install skill. If the team wants persistent repo-scoped custom agents, point them to the official Codex subagents docs and `.codex/agents/*.toml`. |
