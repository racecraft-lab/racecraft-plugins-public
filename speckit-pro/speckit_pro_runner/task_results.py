...
def digest_field(value: Any, name: str) -> str:
    value = text_field(value, name)
    if not re.fullmatch(r"[0-9a-f]{64}", value):
        raise ValueError(f"{name} must be a sha256 digest")
    return value


def partition_sha256(batches: Any, task_units: Any) -> str:
    """Bind the complete frozen dispatch plan for parent-retained comparison."""
    plan = {"batches": batches, "task_units": task_units}
...
def start_journal(root: Path, inputs: dict[str, Any], tasks: Path, path: Path,
    ...
    if others or prior is not None:
        ...
        validate_journal(root, prior_value)
...
def validate_journal(root: Path, journal: dict[str, Any], metadata: dict[str, Any] | None = None) -> None:
    required = {"schema_version", "tasks_file", "fingerprints", "metadata_sha256", "partition_sha256",
                "batches", "task_units", "reports"}
    lineage = {"prior_journal_file", "prior_journal_sha256", "reconciliation_event_id", "reconciliation_reason"}
    if (journal.get("schema_version") != SCHEMA or not required <= journal.keys()
            or journal.keys() - required not in (set(), lineage)):
    ...
    if digest_field(journal["partition_sha256"], "partition_sha256") != partition_sha256(
            journal["batches"], journal["task_units"]):
        raise ValueError("stored partition digest does not match the frozen batch plan")
    seen = []
    for ordinal, batch in enumerate(journal["batches"], 1):
        ...
        seen.extend(tasks)
        if any(t not in journal["task_units"] for t in tasks):
            raise ValueError("frozen task identity does not match its task-unit plan")
        if metadata is not None and any(
                t not in metadata["tasks"] or journal["task_units"].get(t) != metadata["tasks"][t]["tdd_unit"]
                for t in tasks):
...
def validate_lineage(root: Path, path: Path, journal: dict[str, Any]) -> None:
    """Verify every retained predecessor by canonical sibling path and stored bytes."""
    expected_tasks = journal["tasks_file"]
    current = journal
    seen = {path.resolve(strict=False)}
    depth = 0
    while True:
        prior = current.get("prior_journal_file")
        if prior is None:
            return
        if depth >= MAX_LINEAGE_DEPTH:
            raise ValueError(f"task result journal lineage exceeds {MAX_LINEAGE_DEPTH} retained generations")
        depth += 1
        prior_path = result_path(root, prior)
        if (prior_path.parent != path.parent or prior_path.suffix != ".json"
                or not re.fullmatch(r"[A-Za-z0-9_-]+\.json", prior_path.name)):
            raise ValueError("prior journal must be a canonical sibling JSON file")
        identity = prior_path.resolve(strict=False)
        if identity in seen:
            raise ValueError("task result journal lineage contains a cycle")
        seen.add(identity)
        retained = source_snapshot(root, prior_path)
        expected_digest = digest_field(current.get("prior_journal_sha256"), "prior_journal_sha256")
        if retained["digest"] != expected_digest:
            raise ValueError("retained prior journal digest mismatch")
        current = decode_object(retained["content"])
        if current.get("tasks_file") != expected_tasks:
            raise ValueError("prior journal lineage belongs to a different tasks file")
        validate_journal(root, current)
...
def load_bound_journal(root: Path, path: Path, inputs: dict[str, Any], binding: dict[str, Any],
    ...
    if expected is not None:
        expected = digest_field(expected, "expected_partition_sha256")
    if not snapshot["exists"]:
        if expected is not None:
            raise ValueError("parent-bound task result journal is missing; reconcile without resetting its partition")
        return None, snapshot
    if expected is None:
        raise ValueError("existing journal requires parent-retained expected_partition_sha256")
    ...
    if journal["partition_sha256"] != expected:
        raise ValueError("frozen partition does not match parent-retained expected_partition_sha256")
...
def task_results(root: Path, inputs: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    try:
        with lock:
            ...
            journal, snapshot = load_bound_journal(root, path, inputs, binding, metadata)
            changed = False
            if journal is None and action == "start":
