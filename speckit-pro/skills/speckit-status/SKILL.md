---
name: speckit-status
description: "Shows the SpecKit project status dashboard: completed specs, in-progress specs, ready-to-start specs with no blockers, blocked specs with reasons, and a recommendation for the next spec to implement. Reads the technical roadmap, workflow files, and design concept docs. Surfaces archive extension state and Archive Sweep cleanup safety when present. Use when the user says "speckit status", "what is the status", "where am I", "what's next", "project roadmap", "project health", "show me the dashboard", "workflow progress", "which spec should I start", "what should I work on next", "check SPEC-XXX", "how is SPEC-XXX going", "is SPEC-XXX blocked". Read-only — never mutates the repo."
