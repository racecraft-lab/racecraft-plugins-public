...
In speckit-pro, grill-me runs **automatically inside `/speckit-pro:scaffold-spec`** before the workflow file is written, and its output enriches every phase prompt downstream. It is also available standalone via `/speckit-pro:grill-me <input>` for raw briefs, transcripts, or PRD-level scoping before the technical roadmap is created.
