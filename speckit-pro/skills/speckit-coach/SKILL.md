...
| "configure autopilot", "settings", "consensus mode" | Guide to `.claude/speckit-pro.local.md` settings — consensus mode, auto-commit, gate failure behavior, post-impl mode. See [autopilot guide](./references/autopilot-guide.md) |
| "agent teams", "post-impl teams mode", "post-impl-mode", "parallel post-impl", "experimental teams" | Explain the experimental `post-impl-mode: teams` opt-in — delegates post-implementation tasks 10-14 to a 3-teammate Agent Team for parallel verify+review+integration. Requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` env var and Claude Code v2.1.32+; falls back to subagents mode if unavailable. Claude-Code-only (Codex uses sequential spawn_agent). Full design in [post-implementation.md](../speckit-autopilot/references/post-implementation.md) §Mode Selection. |
