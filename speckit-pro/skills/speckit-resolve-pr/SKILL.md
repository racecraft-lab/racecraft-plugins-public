...
description: "Addresses all unresolved review comments on a GitHub pull request: parses thread metadata via gh GraphQL, partitions threads by file and runs file-level fixes as parallel background subagents, posts replies, and resolves each review thread via gh API. Use when the user says "resolve PR review comments", "address review feedback", "fix copilot comments", "resolve threads on PR #X", "address review threads", "fix the review feedback on PR #X", "can you handle the copilot review on this PR", "work through review comments on this PR", or pastes a PR URL with unresolved threads. Mutates code and pushes — confirms before destructive operations. Not for general PR review (no review-creation here)."
