...
description: "Fixes the code that GitHub PR review comments flagged, then resolves each thread. Mutates files and pushes commits — this is NOT a read-only PR review skill. Reads unresolved threads via gh GraphQL, partitions by file, runs file-level fixes as parallel background subagents, runs project verification, posts a reply per thread, and marks each thread resolved via gh API. Use when the user says \"resolve PR review comments\", \"address review feedback\", \"fix the copilot comments\", \"resolve the threads on PR #X\", \"address the review threads\", \"handle the Copilot review on this PR\", \"work through the review comments\", \"fix each review comment and resolve the threads\", \"my PR got review feedback, address them all\", \"take care of the outstanding review feedback\", or pastes a PR URL with unresolved threads. Not for read-only PR review or diagnosing what a PR changes — use a review skill for that."
