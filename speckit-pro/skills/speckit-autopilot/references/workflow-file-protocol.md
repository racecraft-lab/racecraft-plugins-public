...
| Message | Meaning |
| --- | --- |
| `autopilot state workflow_file is not a normalized repository-relative path` | The value is machine-written, so the state is untrustworthy, not the workflow |
| `workflow file is outside the authorized repository` | The supplied workflow resolves outside the repository that holds the state, possibly through a symlink |
| `supplied workflow does not match autopilot state workflow_file authority` | The run is resuming the wrong specification. Both compared paths follow the message: re-point the run, or reclaim the state slot and let the next invocation rewrite it |
