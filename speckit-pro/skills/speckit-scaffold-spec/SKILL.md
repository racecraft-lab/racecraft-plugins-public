...
description: "Scaffolds a new spec for autopilot execution. Reads the technical roadmap, creates a git worktree, pushes the branch, runs a grill-me design interview, and writes a populated workflow file plus a Design Concept doc. Use when the user says "set up SPEC-XXX", "scaffold spec", "scaffold SPEC-XXX", "prepare spec for autopilot", "set up a spec", "create worktree for spec", "start working on SPEC-XXX", "begin SPEC-XXX", "populate the workflow file for SPEC-XXX", "scaffold a new spec from the roadmap". Strictly interactive — requires a human to answer the grill-me questions. Not for ad-hoc design interviews on raw briefs (use /speckit-pro:grill-me) or running the populated workflow (use /speckit-pro:autopilot)."
