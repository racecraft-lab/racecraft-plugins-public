...
via the skill sigil. The command is interactive — it will generate
...
1. **Invoke the command.** Run `$speckit-clarify` with the provided
   workflow prompt.
...
3. **Research before answering.** For each question, use the
   ...
   c. **Codebase exploration** — explore the codebase for
      ...
      - Preferred: `context_builder` + `file_search`
