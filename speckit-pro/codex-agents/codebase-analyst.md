...
- **Broad pattern matching** across the codebase
  - Preferred: `file_search`
  - Fallback: search the codebase with regex patterns
- **API surface exploration** — understand function/type
  signatures without reading full files
  - Preferred: `get_code_structure`
  - Fallback: search for function/class/type definitions
- **Deep code exploration** — understand relationships and
  context across related files
  - Preferred: `context_builder`
  - Fallback: find files matching structural patterns + read content
- Search for specific pattern searches across files (always available)
- Find files matching structural patterns (always available)
