#!/usr/bin/env python3
"""Layer 7 parity fixture runner.

Validates Layer 7 parity fixtures in dry-run mode by default. Live mode runs
...
def print_usage() -> None:
    print(
        "\n".join(
            [
                "Layer 7 - Parity Fixtures Runner",
                "",
                "Usage:",
                ...
                "  L7_OUT                 live-mode output root (default: platform temp directory)",
            ]
        )
    )
...
def run_fixture_live(fixture_dir: Path, config: Config, counts: Counts) -> None:
    ...
    default_out = Path(tempfile.gettempdir()) / f"l7-parity-{os.getpid()}"
    out_root = Path(os.environ.get("L7_OUT", str(default_out))) / fixture_id
...
def main(argv: list[str] | None = None) -> int:
    ...
    counts = Counts()
    print(f"Layer 7: Parity Fixtures (mode={config.mode})")
    print(RULE)
