# Layer 6 — Integration Fixtures (Dispatch Graph)
...
These are all **dispatch graph** failures. Layer 6 exists to catch them.
...
python3 tests/speckit-pro/layer6-integration/run-all-fixtures.py

# Replay just Class 1
python3 tests/speckit-pro/layer6-integration/run-dispatch-fixtures.py

# Replay one fixture, verbose
VERBOSE=true python3 tests/speckit-pro/layer6-integration/run-dispatch-fixtures.py 03-redelegation-chain

# Live capture for one fixture (costs LLM tokens)
python3 tests/speckit-pro/layer6-integration/run-dispatch-fixtures.py --live 01-clarify-codebase-only

# Live capture across the whole layer
python3 tests/speckit-pro/layer6-integration/run-all-fixtures.py --live
```
...
Per the dispatch architecture audit (PR #26), Layer 6 uses **structural
...
```bash
python3 tests/speckit-pro/layer6-integration/scrub-transcript.py path/to/transcript.jsonl
...
Layer 6 replay fixtures still cover Claude Code dispatch transcripts only.
