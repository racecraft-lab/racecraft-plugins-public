#!/usr/bin/env python3
"""Run every Layer-6 fixture class."""
...
def parse_args(argv: list[str]) -> tuple[str, str]:
    ...
    if selected != "all" and selected not in RUNNERS:
        raise ValueError(f"unknown Layer-6 class: {selected}")
...
def run_class(class_id: str, mode: str) -> bool:
    ...
    if not passed:
        print(f"FAIL Layer 6 class {class_id}: {detail}", file=sys.stderr)
    return passed
...
def main(argv: list[str]) -> int:
    ...
    print("\n" + "=" * 66)
    print(f"  Layer 6 {'PASSED' if passed else 'FAILED'}")
    print("=" * 66)
