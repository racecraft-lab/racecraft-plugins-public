...
def ambiguity_case() -> dict[str, object]:
    ...
    return {
        ...
        "prompt": "Use {{skill}} for workflow docs/ai/specs/.process/SPEC-107-workflow.md. Complete its workflow-binding preflight with the protected installed runner using exactly python3 -m speckit_pro_runner < scenario-inputs/binding-request.json. Interpret its returned status, including an expected-failure exit, rather than replacing its response. Return one JSON object with binding_result (the complete actual helper response), decision (proceed or stop), candidates (canonical worktree roots), and next_action (continue_workflow or provide_absolute_workflow_path). Stop at the preflight decision; do not start later workflow phases.",
        "fixtures": [
