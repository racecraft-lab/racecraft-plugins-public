...
class SurfaceConfinementTests(unittest.TestCase):
    ...
    def test_plugin_packages_the_local_broker_and_receipt_schema(self) -> None:
        ...
        author_server = manifest["mcpServers"]["author-broker"]
        self.assertEqual(["-m", "speckit_pro_runner.author_broker"], author_server["args"])
        self.assertEqual("${CLAUDE_PLUGIN_ROOT}", author_server["env"]["PYTHONPATH"])
        self.assertEqual(
            manifest,
            json.loads(
