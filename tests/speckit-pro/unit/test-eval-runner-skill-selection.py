...
class EvalRunnerSkillSelectionTests(unittest.TestCase):
    ...
    def test_codex_autopilot_worktree_handoff_contract(self) -> None:
        ...
        self.assertIn("Open a new Codex task rooted at <worktree>", scaffold)
        ...
        self.assertIn("Open a new Codex task rooted at <workflow_root>", prerequisites)
        self.assertIn("exact absolute workflow command", prerequisites)
        self.assertIn("original stage flags", prerequisites)
        self.assertNotIn("Use Codex Handoff to move this task to", prerequisites)
