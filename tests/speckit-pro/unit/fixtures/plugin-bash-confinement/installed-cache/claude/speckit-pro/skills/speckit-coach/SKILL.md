...
| "agent teams", "parallel post-impl", "speckit-pro and agent teams", "does speckit use teams" | Explain that Step 0.6 uses `resolve-claude-subagent-runtime`. Teams require the env flag, Claude Code ≥ 2.1.178, positively interactive execution, and exact-client live UAT. Headless runs and any unverified teammate contract use bounded parallel ordinary subagents. Team starts use named `Agent` calls; ordinary calls omit `name`. Full design in [post-implementation.md](../speckit-autopilot/references/post-implementation.md) §Post-Implementation Parallel Group. |
| "when to use teams", "subagents vs teams", "agent teams vs subagents", "which orchestration mode", "decision framework" | Walk through the canonical decision tree: one bounded task → foreground subagent; independent disjoint work → bounded background-subagent waves; independent sessions that must communicate → runtime/UAT-gated Agent Team; overlapping or unknown ownership → serial; headless → ordinary subagents. Never assume subagent-only `disallowedTools`, `memory`, `maxTurns`, `skills`, or effort settings apply to teammates. Full table in [agent-teams-integration.md](../speckit-autopilot/references/agent-teams-integration.md) §When to use what. |
| "headless", "overnight", "budget", "max budget", "background autopilot", "run autopilot from cron", "schedule autopilot" | Explain headless autopilot — `claude -p --max-budget-usd 25 /speckit-pro:speckit-autopilot path/to/workflow.md`. The budget cap applies to the parent session AND all subagents/teammates collectively. A 7-phase run with consensus + parallel post-impl + every agent at `effort: max` (or lower effort admitted by one of two paths: L6-validated quality=1.0, or a role contract that forbids interpretation and evidence gathering and returns a program-parsed shape) typically spends more than a cost-optimized setup — size the budget to match the plugin's effort policy. See [agent-teams-integration.md](../speckit-autopilot/references/agent-teams-integration.md) §Headless / budget-capped operation. |
...
| "upgrade speckit", "update speckit", "update templates", "new version", "latest version" | Hand off to `/speckit-pro:speckit-upgrade` (snapshot `.specify/`, then `specify integration upgrade`, then restore; never `init --here --force` first). |
...
| "technical roadmap", "decompose feature", "multi-spec", "too large for one spec" | Guide technical roadmap creation — decompose large features into sequential specs. See Enhancement section below. **Tip:** if no PRD exists yet, author it and the roadmap together with `/speckit-pro:speckit-prd`. To sharpen an existing PRD before decomposing, run `/speckit-pro:grill-me docs/prd.md` first to lock in the scope envelope. |
...
- **Upgrade with `/speckit-pro:speckit-upgrade`** — it snapshots `.specify/` before touching it
