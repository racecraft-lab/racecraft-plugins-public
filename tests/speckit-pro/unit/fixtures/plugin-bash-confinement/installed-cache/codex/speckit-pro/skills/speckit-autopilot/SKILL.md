...
4. **3.2 PR Creation** — final verification, then apply the final
   ...
   blocked with validator evidence because split packet output remains deferred. For
   ...
   `gh pr create/edit` fallback before any stack-manager mutation. Commit the
   generated packet artifacts, reverify and revalidate them, push, create the
   PR, then update the workflow file.
