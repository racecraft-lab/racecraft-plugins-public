...
class BatchedTaskResultsTests(unittest.TestCase):
    ...
    def call(self, action="start", mode="apply", **inputs):
        request = {**self.inputs, "action": action, **inputs}
        journal_file = request["journal_file"]
        if journal_file in self.partition_bindings and "expected_partition_sha256" not in request:
            request["expected_partition_sha256"] = self.partition_bindings[journal_file]
        result = task_results(self.root, request, mode)
        if action == "start" and mode == "apply":
            self.partition_bindings[journal_file] = result["partition_sha256"]
        return result
    ...
    def test_existing_journal_requires_parent_retained_partition_binding(self):
        report = self.first_report()
        expected = self.partition_bindings[self.inputs["journal_file"]]
        for action, mode, extra in (
            ("start", "apply", {}),
            ("inspect", "read_only", {}),
            ("record", "apply", report),
        ):
            with self.subTest(action=action, binding="missing"), self.assertRaises(ValueError):
                task_results(self.root, {**self.inputs, "action": action, **extra}, mode)
            with self.subTest(action=action, binding="mismatch"), self.assertRaises(ValueError):
                task_results(self.root, {**self.inputs, "action": action,
                    "expected_partition_sha256": "0" * 64, **extra}, mode)
        self.assertEqual(self.call("inspect", mode="read_only")["partition_sha256"], expected)

    def test_recomputed_self_hash_cannot_hide_frozen_route_tampering(self):
        journal = self.call()["journal"]
        expected = self.partition_bindings[self.inputs["journal_file"]]
        journal["batches"][0]["agent"] = "speckit-pro:domain-researcher"
        journal["batches"][0]["tdd_not_applicable_reason"] = non_tdd_reason(journal["batches"][0])
        journal["partition_sha256"] = partition_sha256(journal["batches"], journal["task_units"])
        self.assertNotEqual(journal["partition_sha256"], expected)
    ...
    def test_successor_rejects_missing_or_modified_retained_history(self):
        ...
        successor = "feature/.process/task-results/reconciled.json"
        started = self.call(journal_file=successor, prior_journal_file=self.inputs["journal_file"],
                            reconciliation_event_id="native-parent-reconcile-1", reconciliation_reason="replanned")
        self.path.write_text("{}\n")
        with self.assertRaises(ValueError):
            self.call("inspect", mode="read_only", journal_file=successor)
        self.path.write_bytes(prior_bytes)
        self.path.unlink()
        with self.assertRaises(ValueError):
            self.call("record", journal_file=successor, **self.report(started["journal"]["batches"][0]))

    def test_successor_creation_rejects_malformed_prior_journal(self):
        self.call()
        self.path.write_text("{}\n")
        with self.assertRaises(ValueError):
            self.call(journal_file="feature/.process/task-results/reconciled.json",
                      prior_journal_file=self.inputs["journal_file"],
                      reconciliation_event_id="native-parent-reconcile-1", reconciliation_reason="replanned")

    def test_multigeneration_lineage_survives_changed_old_fingerprints(self):
        self.call()
        first_bytes = self.path.read_bytes()
        self.body = self.body.replace("behavior 1", "changed behavior 1")
        (self.feature / "tasks.md").write_text(self.body)
        self.meta["fingerprints"] = fingerprints("spec\n", "plan\n", self.body)
        self.metadata_path.write_text(json.dumps(self.meta))
        second = "feature/.process/task-results/reconciled-one.json"
        self.call(journal_file=second, prior_journal_file=self.inputs["journal_file"],
                  reconciliation_event_id="native-parent-reconcile-1", reconciliation_reason="definitions changed")
        third = "feature/.process/task-results/reconciled-two.json"
        self.call(journal_file=third, prior_journal_file=second,
                  reconciliation_event_id="native-parent-reconcile-2", reconciliation_reason="continued reconciliation")
        self.assertEqual(self.call("inspect", mode="read_only", journal_file=third)["journal_path"], third)
        self.assertEqual(self.path.read_bytes(), first_bytes)

    def test_lineage_cycle_and_depth_limit_fail_closed(self):
        journal = self.call()["journal"]
        journal.update({"prior_journal_file": self.inputs["journal_file"],
                        "prior_journal_sha256": "0" * 64,
                        "reconciliation_event_id": "cycle", "reconciliation_reason": "cycle"})
        self.path.write_text(json.dumps(journal))
        with self.assertRaises(ValueError):
            self.call("inspect", mode="read_only")

        self.path.write_text(json.dumps({key: value for key, value in journal.items()
                                        if key not in {"prior_journal_file", "prior_journal_sha256",
                                                       "reconciliation_event_id", "reconciliation_reason"}}))
        prior = self.inputs["journal_file"]
        for generation in range(1, MAX_LINEAGE_DEPTH + 1):
            current = f"feature/.process/task-results/generation-{generation}.json"
            self.call(journal_file=current, prior_journal_file=prior,
                      reconciliation_event_id=f"generation-{generation}", reconciliation_reason="bounded lineage")
            prior = current
        with self.assertRaises(ValueError):
            self.call(journal_file="feature/.process/task-results/over-limit.json", prior_journal_file=prior,
                      reconciliation_event_id="over-limit", reconciliation_reason="bounded lineage")
    ...
    def runner(self, action="start", mode="apply", **inputs):
        ...
        request_inputs = {**self.inputs, "action": action, **inputs}
        journal_file = request_inputs["journal_file"]
        if journal_file in self.partition_bindings and "expected_partition_sha256" not in request_inputs:
            request_inputs["expected_partition_sha256"] = self.partition_bindings[journal_file]
        request = {"schema_version": "1.0", "helper_id": "task-results", "operation": "task-results",
                   "mode": mode, "inputs": request_inputs}
        ...
        if action == "start" and mode == "apply" and process.returncode == 0:
            self.partition_bindings[journal_file] = result["data"]["partition_sha256"]
