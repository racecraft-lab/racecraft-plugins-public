...
#
# Usage:
#   bash run-dispatch-fixtures.sh                           # all fixtures, replay
#   bash run-dispatch-fixtures.sh 01-clarify-codebase-only  # one fixture, replay
#   bash run-dispatch-fixtures.sh --live                    # all fixtures, live
#   bash run-dispatch-fixtures.sh --live 03-redelegation-chain
#
...
PLUGIN_ROOT="$(cd "$TESTS_ROOT/../../speckit-pro" && pwd)"
LIB="$SCRIPT_DIR/lib/transcript-helpers.sh"
FIXTURES_DIR="$SCRIPT_DIR/dispatch-fixtures"
...
MODE="replay"
SELECTED=""
BUDGET_USD="${DISPATCH_FIXTURE_BUDGET_USD:-1.00}"
...
section "Layer 7 Class 1: Dispatch Fixtures (mode: $MODE)"
