...
def _recover_append_only_target(path, payload, expected_parent_identity=None):
    ...
    try:
        ...
        current_target = os.stat(target.name, dir_fd=parent_descriptor, follow_symlinks=False)
        if (
            not stat.S_ISREG(target_before.st_mode)
            or _stable_file_identity(current_target) != _stable_file_identity(target_before)
        ):
            raise ValueError("append-only recovery target changed before inspection")
        if target_before.st_nlink == 1:
            return False
        retained = _read_open_descriptor(target_descriptor)
        target_after = os.fstat(target_descriptor)
        if (
            retained != payload
            or _stable_file_identity(target_before) != _stable_file_identity(target_after)
        ...
        if target_after.st_nlink == 1:
            return False
        if target_after.st_nlink != 2:
            raise ValueError("append-only target retains unrecognized alternate hard links")
        candidates = []
        for name in os.listdir(parent_descriptor):
            if not _APPEND_ONLY_TEMPORARY_NAME.fullmatch(name):
                continue
            metadata = os.stat(name, dir_fd=parent_descriptor, follow_symlinks=False)
            if (metadata.st_dev, metadata.st_ino) == (target_after.st_dev, target_after.st_ino):
        ...
        if _stable_file_identity(temporary_metadata) != _stable_file_identity(target_after):
...
def _recover_content_addressed_append_only_links(raw):
    ...
    directories.extend(
        Path(raw) / name for name in (
            RETENTION_RECORDS_DIR, PUBLICATION_RECEIPTS_DIR,
    ...
    for directory in directories:
        ...
        try:
            if _stable_directory_identity(os.fstat(descriptor)) != identity:
            ...
            for temporary_name in names:
                ...
                if not stat.S_ISREG(temporary.st_mode) or stat.S_IMODE(temporary.st_mode) != 0o600:
                    raise ValueError("append-only recovery temporary must be a private regular file")
                if temporary.st_nlink == 1:
                    raise ValueError("append-only recovery temporary lacks a published target")
                targets = []
                for name in names:
                    ...
                    if (candidate.st_dev, candidate.st_ino) == (temporary.st_dev, temporary.st_ino):
                        targets.append(name)
                if temporary.st_nlink != 2 or len(targets) != 1 or not _HEX_SHA256.fullmatch(Path(targets[0]).stem):
                    raise ValueError("append-only temporary link lacks one content-addressed target")
                target = directory / targets[0]
                payload = _read_bounded_regular_file(
                    target, allowed_root=directory, expected_parent_identity=identity,
                )
                if target.name != f"{digest(payload).removeprefix('sha256:')}.json":
                    raise ValueError("append-only recovery target is not content-addressed")
                _recover_append_only_target(target, payload, identity)
        finally:
            os.close(descriptor)
