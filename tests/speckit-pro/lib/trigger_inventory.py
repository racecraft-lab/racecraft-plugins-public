...
def validate_active(inventory: dict, layer_root: Path) -> dict:
    ...
    for row in inventory["active"]:
        ...
        if row["category"] == "explicit":
            require("$" not in row["query"] and not row["query"].lstrip().startswith("/"), "explicit smoke must not use native control syntax")
            target = f"speckit-pro:{row['skill']}" if row["host"] == "claude" else row["skill"]
            require(f"use the {target} skill" in row["query"].casefold(), "explicit smoke must clearly name the exact target skill")
        active[identity] = row
