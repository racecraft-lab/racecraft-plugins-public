...
set_test "Claude reference routes split-PR post-impl through multi-pr-emission"
assert_contains "$claude_body" "multi-pr-emission" "Claude reference must name the shared emitter"

set_test "Claude reference consumes the PRSG-008 layer plan without new slicing heuristics"
assert_contains "$claude_body" "plan-layers"
...
assert_contains "$claude_body" "restack"
...
assert_contains "$codex_body" "multi-pr-emission"
assert_contains "$codex_body" "plan-layers"
...
assert_contains "$codex_body" "restack"
...
assert_contains "$claude_body" "validate-uat-runbook"
...
assert_contains "$codex_body" "validate-uat-runbook"
...
assert_contains "$codex_dist_body" "multi-pr-emission"
