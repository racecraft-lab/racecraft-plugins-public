...
FIXTURE_DIR = Path(__file__).resolve().parent / "fixtures" / "xplat-010-confinement"
...
TEMP_ALLOWLIST = "tests/speckit-pro/layer4-scripts/fixtures/xplat-010-confinement/allowlist.json"
...
class RepoBashConfinementDurabilityTests(unittest.TestCase):
    def test_registry_release_and_default_suite_composition(self) -> None:
        ...
        members = [item for item in layer4["scripts"] if item["path"] == "tests/speckit-pro/layer4-scripts/test-repo-bash-confinement.py"]
