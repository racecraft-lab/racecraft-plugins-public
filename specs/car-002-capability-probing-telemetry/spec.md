...
- **Partial probe matrix**: The snapshot is one atomic committed artifact — no partial snapshot is ever written. Three dispositions: (1) any observation failing schema validation, an unparseable `--output-format json` payload, or an uninterpretable plain-text effort-acceptance output (the expected clamp-warning presence/absence cannot be parsed) aborts the whole snapshot write (fail-closed — a silently-omitted tuple would make the SC-005 join resolve routes to zero tuples); (2) an infrastructure/transport failure with no interpretable platform signal (non-zero exit with no parseable error body, timeout, network failure) aborts the run with nothing committed, and is NOT recorded as "unavailable" — recording a transport failure as unavailable would falsely narrow platform availability (FR-026); there are no automatic retries, so a transport failure always surfaces to the operator for a rerun; (3) an interpretable platform observation — an unavailable-model result, the models endpoint unreachable in subscription mode, a question unanswerable within the bounded matrix, or an undetermined unavailable-probe outcome (parseable payload whose outcome path cannot be classified among the documented paths) — is recorded as an explicit unavailable/gap/open/undetermined observation and the snapshot is written; no availability claim derives from an undetermined observation (CAP-Q5 stays open).
...
- **FR-003**: The probe matrix MUST dedupe the 37 candidate routes to unique (model, effort) tuples, executing one alias-canary invocation per unique model alias (for ID binding, CAP-Q1..Q4) plus one configuration-acceptance check per unique (model, effort) tuple, bounded at roughly 20 live invocations worst case. Every live invocation MUST run under an explicit per-invocation timeout (its value recorded in snapshot probe metadata), and there are NO automatic retries — a timed-out or transport-failed invocation follows the "Partial probe matrix" edge case's abort disposition — so the bound counts actual live invocations and equals the precomputed matrix cardinality (an overrun is surfaced before any live call).
...
- **FR-012**: For each probe, the snapshot MUST store the complete sanitized stdout of that probe invocation as raw evidence — `--output-format json` stdout for the alias-canary and unavailable-model probes, or plain-text `--print` stdout for the effort-acceptance probes (which run in plain text to avoid the silent JSON effort clamp) — with all home/user paths normalized to `<home>` per the existing release-readiness sanitization convention. Raw evidence is therefore not always JSON-parsable; consumers key parsing on the probe's recorded output mode.
