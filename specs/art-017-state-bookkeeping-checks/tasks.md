...
**Reviewability**: Preserve the accepted one-slice budget: authored implementation stays limited to `speckit-pro/skills/speckit-autopilot/scripts/validate-autopilot-phase-coverage.py`, both `speckit-pro/skills/speckit-autopilot/SKILL.md` and `speckit-pro/codex-skills/speckit-autopilot/SKILL.md`, and `tests/speckit-pro/unit/test-autopilot-bookkeeping-guard.py`. Client payloads, installed-cache proofs, and docs references are regenerated by tooling and never hand-edited.
...
- [X] T006 Locate the corresponding authored autopilot guidance paragraph in each client source skill, leaving generated payloads and fixtures untouched for FR-016 and FR-017.
...
- [X] T021 [US2] Narrow the corresponding authored guidance paragraph in both client source skills so legacy structural coverage debt stays distinct from the three blocking current-run state invariants for FR-016; the Codex source correction was completed through post-review remediation.
- [X] T022 [US2] Review `git diff` for the validator, both client source skills, and the bookkeeping-guard test to confirm authored edits stayed inside the corrected four-file boundary for FR-016 and FR-017.
...
**Acceptance Criteria**:
- FR-017 client payloads, installed-cache fixtures, proofs, and references are tooling-owned; both client source skills are authored.
...
- Claude Code and Codex release payloads, installed-cache proofs, and docs references are refreshed by tooling only; both client distributions must pass final-authored-tree consistency evidence.
