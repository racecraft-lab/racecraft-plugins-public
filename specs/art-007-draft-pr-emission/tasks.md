...
- [ ] T015 [US1] Make the producer's packet assembly mode-aware in `speckit-pro/speckit_pro_runner/helpers/pr_emission.py` at **all four** hardcoded reviewer-packet sites, not just the heading set — an unconditional site makes `pr-packet-output` emit a draft packet its own schema rejects: give `required_headings()` a `mode` parameter (draft returns `Artifacts`, `Resume`; every other value returns today's eight in today's order); give `editable_fields()` a `mode` parameter returning `[]` for draft; set `uat.uat_runbook_heading` to `""` for draft; and set `protected_body_fingerprint.elided_fields` to `[]` for draft. The call sites already thread these through as packet data, so the body structure checker needs no change at all. `build_packet_body`'s fallback stays single/split-shaped and is never reached in draft mode, because the orchestrator passes the finished body as `inputs.body`. Add no `split_slice` analogue for draft — draft mode attaches no new required sub-object (FR-008)
