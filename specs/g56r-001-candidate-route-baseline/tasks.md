...
- [x] T001 [US1] Create the research-boundary header in `docs/ai/research/codex-agent-route-candidates.md` and record RFC 3339 `started_at` plus the scheduled `deadline_at` before any checker or evidence work begins (AC-1.5)
- [x] T002 [US1] Write focused missing-artifact, fixed-path, offline, and read-only assertions in `tests/speckit-pro/unit/test-g56r-001-artifacts.py`, run that file directly, and record verified RED assertion failures before the checker exists (AC-1.5, AC-1.6)
- [x] T003 [US1] Declare `tests/speckit-pro/unit/test-g56r-001-artifacts.py` in Layer 4 of `tests/speckit-pro/suite-manifest.json` without changing any other layer or test entry (AC-1.5)
- [x] T004 [US1] Implement the fixed artifact paths, manifest envelope, exact 12/10/2 and 3/9 sets, four surfaces, one-day timestamp rules, sanitization prohibitions, and precise missing-artifact behavior in `specs/g56r-001-candidate-route-baseline/check-artifacts.py`, then rerun T002 to verified GREEN (AC-1.1, AC-1.4, AC-1.5, AC-1.6, AC-1.7)
- [x] T005 [US1] Add canonicalization, readable-ID, hash-format, manifest-content-hash omission, route-to-contract binding, candidate tuple/control, and eligibility-versus-availability assertions to `tests/speckit-pro/unit/test-g56r-001-artifacts.py`, then run it and record verified RED assertion failures (AC-1.3, AC-1.6)
- [x] T006 [US1] Implement the T005 canonicalization, identity, self-reference-free manifest hash, binding, candidate, and availability checks in `specs/g56r-001-candidate-route-baseline/check-artifacts.py`, then rerun the focused unit file to verified GREEN (AC-1.3, AC-1.6)
- [x] T007 [US1] Add assertions for route-policy inventory links, provenance, freshness, independent surfaces, source observations, parity mappings, fixture contracts, telemetry, classified unknowns, cross-artifact agreement, and reproducible handoff to `tests/speckit-pro/unit/test-g56r-001-artifacts.py`, then record verified RED assertion failures (AC-1.1, AC-1.2, AC-1.3, AC-1.4, AC-1.5, AC-1.6, AC-1.7)
- [x] T008 [US1] Implement the T007 fixed-shape and agreement checks in `specs/g56r-001-candidate-route-baseline/check-artifacts.py`, rerun the focused unit file to verified GREEN, then run the checker and confirm its only remaining failure is the not-yet-complete narrative and manifest (AC-1.1, AC-1.2, AC-1.3, AC-1.4, AC-1.5, AC-1.6, AC-1.7)
...
- [x] T014 [US1] Derive complete FR-006 semantic contracts and immutable tracked production routes for the ten current Codex agents from `speckit-pro/codex-agents/`, recording permitted/prohibited behavior and stop/escalation conditions for every hard boundary in `docs/ai/research/codex-agent-route-candidates.md` (AC-1.1, AC-1.3, AC-1.6)
- [x] T015 [US1] Derive `consensus-synthesizer` and `gate-validator` semantic contracts from the corresponding `speckit-pro/agents/` instruction bodies, record all twelve field-level parity mappings and justified Codex adaptations, and record explicit absent Codex production routes in `docs/ai/research/codex-agent-route-candidates.md` (AC-1.1, AC-1.3, AC-1.6)
- [x] T016 [US1] Calculate and record one stable contract ID plus repeatable instruction and contract hashes for each of the twelve contracts, and bind each present route to its enclosing contract without encoding preference rank in `docs/ai/research/codex-agent-route-candidates.md` (AC-1.3, AC-1.6)
- [x] T017 [US1] Enumerate every evidence-supported project-level model/effort/treatment candidate, retain each immutable production baseline and each justified unchanged control, cite all inclusion or hard-contract exclusion evidence, keep installation availability `unresolved_g56r_002`, and label all preference signals as unqualified hypotheses in `docs/ai/research/codex-agent-route-candidates.md` (AC-1.2, AC-1.3, AC-1.4, AC-1.6)
- [x] T018 [US1] Define actionable fixture contracts for all twelve agents, classifying exactly `codebase-analyst`, `domain-researcher`, and `spec-context-analyst` as current and the other nine as missing, with repository-relative paths or null, representative inputs, expected outputs, hard-contract assertions, and historical prompt emulation labeled `non_release_evidence` in `docs/ai/research/codex-agent-route-candidates.md` (AC-1.3, AC-1.7)
- [x] T019 [US1] Record per-agent telemetry requirements and classify every remaining question as closed G56R-001 documentation/inventory work, G56R-002 executable-capability work, or G56R-003 replay/scoring/qualification work with impact, exact owner, and required follow-up in `docs/ai/research/codex-agent-route-candidates.md` (AC-1.4, AC-1.5, AC-1.7)
...
- [x] T020 [US1] Create the UTF-8 agent-centric envelope and twelve lexically presented agent records in `docs/ai/research/codex-agent-route-candidate-manifest.json`, encoding each complete contract, canonical IDs/hashes, and immutable present route or cited absence with explicit nulls (AC-1.1, AC-1.3, AC-1.6)
- [x] T021 [US1] Encode every candidate, capability requirement, rationale, incompatibility, qualification requirement, provenance record, invalidation trigger, source observation, independent surface record, fixture contract, telemetry requirement, and classified unknown in `docs/ai/research/codex-agent-route-candidate-manifest.json`, using explicit empty arrays and permitted nulls where required (AC-1.2, AC-1.3, AC-1.4, AC-1.5, AC-1.6, AC-1.7)
- [x] T022 [US1] Finalize the normalized twelve-agent projection and evidence-backed candidate rationale in `docs/ai/research/codex-agent-route-candidates.md` so it agrees field-for-field with `docs/ai/research/codex-agent-route-candidate-manifest.json` while remaining a cited human review record rather than generated policy (AC-1.1, AC-1.2, AC-1.3, AC-1.4, AC-1.6, AC-1.7)
