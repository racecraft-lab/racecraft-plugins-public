...
## Direction of Preference — Resolved by FR-058

- [x] CHK050 - Is the **direction of preference** declared for each of the eight Pareto dimensions, given that `acceptance` is better when higher, the token and duration dimensions are better when lower, and `terminal_state` is categorical rather than ordered? Without a declared ordering, "no worse on every dimension" is not objectively decidable. [Gap, Measurability, Spec §FR-018] — **Remediated by FR-058**, which was added after this item was first written. The item's original note assumed the only available fix was a `direction` map inside `pareto_policy`, which would indeed have been a unilateral edit to the frozen parity surface. FR-058 takes the parity-safe route instead: direction of preference is comparison **semantics**, not schema shape, so it is fixed in requirement text and in the implementation (T063 asserts it, T072 implements it) while `pareto_policy.dimensions` stays exactly the eight enum strings of FR-018 with no added member — verified against `contracts/analysis-plan.schema.json`. The two platforms therefore agree on which dimensions are compared, and only this side currently states how; FR-058 requires that wording be mirrored to the twin verbatim. A *schema-level* direction map remains a joint change if one is ever wanted, but it is no longer needed to make the comparison decidable.
...
Requirements added this pass: **FR-052** (stratum membership derived pre-execution, powered long-horizon stratum), **FR-053** (guardrail definitional completeness and the guardrail multiplicity family), **FR-054** (per-stratum estimability floor), **FR-055** (racing and futility declaration content, interim-look error control), **FR-056** (campaign-ceiling truncation vs candidate budget exhaustion; no post-freeze relaxation), **FR-057** (default-suite and replay cost bounds), **FR-058** (declared direction of preference per Pareto dimension — see CHK050 above). Success criteria added: **SC-022** through **SC-025**; **SC-019** extended with the runtime bound.

Contracts changed (CAR-owned only): `contracts/analysis-plan.schema.json` (`guardrail_method` including its `units` map, per-stratum `membership_rule` / `stratum_minimum_unique_tasks` / `stratum_sample_size`, `reliability_guardrails`, structured `racing_policy` and `futility_policy`); `contracts/experiment-assignment.schema.json` (`stratum_assignment`); `contracts/car-003-additive-records.schema.json` (`observed_cache_isolation`, `scorerIdentityAttestation`). The parity-mirror contracts `score-bundle.schema.json` and `analysis-decision.schema.json` were **not** modified, and `pareto_policy.dimensions` remains exactly the eight of FR-018.
