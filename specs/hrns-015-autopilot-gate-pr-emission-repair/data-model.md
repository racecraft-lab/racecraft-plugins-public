...
**PostRow**: canonical name from the 13-item `POST_STEPS` source, order, persisted workflow occurrence/status, and state-file occurrence/status. At the success boundary, every canonical row must occur exactly once in both persisted representations with `completed` status. Missing, duplicate, pending, and in-progress rows all fail with named diagnostics. A legacy 11-row record remains incomplete: only exact-name statuses carry forward, while new or renamed canonical rows begin pending and cannot inherit predecessor completion. Workflow `✅ Complete` is the `completed` state value; other workflow statuses cannot satisfy the boundary.
