...
- **FR-018** [US10]: Before Claude or Codex reports successful full autopilot completion after Post, it MUST run a completion-boundary check against the persisted workflow and `autopilot-state.json`. The check MUST require each of the 13 canonical Post steps to appear exactly once with status `completed`, name every missing or noncompleted step, and fail when any requirement is unmet. An older 11-row persisted record MUST fail with its missing canonical names; resume reconciliation MUST preserve each exact-name row status, create missing or renamed canonical rows as pending, and MUST NOT infer their completion from a predecessor row. In the workflow table `✅ Complete` maps to state `completed`; pending, in-progress, and other workflow statuses do not.
...
- The shared completion-boundary rule reads the persisted workflow and `autopilot-state.json` immediately before either host reports successful full completion after Post. A `--stage plan` or `--stage implement` return does not claim that full completion; its out-of-stage rows remain explicitly skipped and the full Post boundary has not been reached. The current `status-evidence` rule does not establish that all 13 canonical Post rows are completed.
