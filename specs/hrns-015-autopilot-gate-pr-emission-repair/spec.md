...
**Created**: 2026-09-25  
**Status**: Draft  
**Input**: Repair observed SpecKit Pro autopilot and scaffold defects on Claude Code and Codex in ordered review increments, each with failing-first fixtures. The complete request is the Phase 1 Detailed Prompt in [HRNS-015-workflow.md](../../docs/ai/specs/.process/HRNS-015-workflow.md).
...
3. **Given** persisted state from the older 11-row Post list, **when** the 13-row guard reads it, **then** it identifies each missing canonical row, retains the status of each exact-name row, and does not infer completion for a new or renamed row.
...
- **FR-001** [US1]: Final packet creation MUST accept an optional `release_note` input as an unfenced Markdown string whose trimmed content is nonempty, without requiring a replacement of the full body. An explicit non-string, blank, or fence-breaking value MUST fail input validation rather than silently act as an omitted note.
- **FR-002** [US1]: When supplied, the renderer MUST wrap the note content in exactly one `release-note` fence in an editable `## Release note` section after the eight required headings, with its heading and balanced marker lines protected, and pass the host's release-note check for a feature PR title.
...
- **FR-018** [US10]: Before Claude or Codex reports successful full autopilot completion after Post, it MUST run a completion-boundary check against the persisted workflow and `autopilot-state.json`. Each of the 13 canonical Post steps MUST appear exactly once in both records with matching terminal status. A row MUST be `✅ Complete` in the workflow and `completed` in state, except a canonical optional extension-dependent Post row MAY use the identical `skipped: <extension> not installed` status in both records only when the named extension is neither enabled in a supported registry nor present as an extension directory. That status encodes the extension name and `extension_not_installed` reason; out-of-stage and every other skip do not qualify. The check MUST fail and name every missing, duplicate, pending, in-progress, mismatched, or unjustifiably skipped row. An older 11-row record MUST fail with its missing canonical names; resume reconciliation MUST preserve status only for unique exact-name rows, initialize missing or renamed canonical rows as pending, and MUST NOT infer completion from row position or a predecessor.
...
- **FR-021** [US11]: On each host, resolve-pr MUST apply fixes, complete full verification, commit, and push before replying to or resolving review threads. A failed verification MUST prevent push, replies, and resolution; if fix commits already exist locally, they remain local and the failure report MUST name the failed gate and retained commit. A failed push, fresh-head query, or mismatch MUST leave review threads pending without replies or resolution and report the retained local commit and failure. A retry MUST complete verification, push, and compare the intended local commit SHA with a freshly retrieved matching remote PR head SHA before serial replies and resolutions; each resolved state MUST be confirmed.
...
- **SC-010**: Each approved PR increment remains at or below 4 production files and below 25 total files, with passing acceptance fixtures and aligned host instructions.
...
- Gap counting recognizes the exact, case-sensitive token `Gap` within a single-line, non-nested `[...]` tag. Split the tag contents on commas and trim spaces and tabs around each token. Count each qualifying tag once, whether `Gap` is the first or a later token. For example, a bracket tag containing only `Gap`, one containing `Gap, Exception Flow`, and one containing `Coverage, Gap` each count once; `[gap]`, `[Gapless]`, and nested bracket text do not count. This is a new Clarify decision; the prior Q4 decision did not settle these boundaries.
...
- A supplied nonblank, unfenced Markdown release-note string is a fourth final-body editable field; explicit non-string, blank, or fence-breaking input is invalid rather than omitted. Its heading and balanced marker lines are protected; only the enclosed body is elided from the protected fingerprint. Structure validation checks the heading and markers and exactly one nonempty release-note fence. Final packet schema permits four fields; drafts retain zero.
...
- The shared completion-boundary rule reads the persisted workflow and `autopilot-state.json` immediately before either host reports successful full completion after Post. A `--stage plan` or `--stage implement` return does not claim that full completion; its out-of-stage rows remain explicitly skipped and the full Post boundary has not been reached. At that boundary, only a verified `skipped: <extension> not installed` status for an optional extension-dependent row can substitute for completion; all other skips block. The current `status-evidence` rule does not establish that the 13 canonical Post rows have reached acceptable terminal states.
