...
- **Scale/Scope**: 29 functional requirements, 14 user stories, 11 success criteria, four stacked review slices. HRNS-019 owns the broad helper-envelope sweep; HRNS-017 owns the unknown Codex child-lifetime behavior.
...
- `docs/prd-harness-engineering-uplift.md` and `docs/ai/specs/harness-engineering-uplift-technical-roadmap.md` — changed: AC-16.2/16.5/16.8/16.10, **both the HRNS-015 and HRNS-019 roadmap entries** (four-slice ownership and deferred envelope sweep), Slice A, and the stale #642 status.
...
- `.specify/quality-gates.json` — changed: optional per-slot `commands` map, preserving approved thresholds and basis.
...
- `speckit-pro/skills/speckit-autopilot/scripts/validate-autopilot-phase-coverage.py` — changed: 13 canonical `POST_STEPS` and a **new** completion-boundary rule against persisted workflow and `autopilot-state.json`. The current `status-evidence` rule is insufficient.
- `speckit-pro/speckit_pro_runner/formal/lifecycle.py` — reviewed and changed only if its hardcoded Post-name subset fails the 13-row contract.
- Both hosts' autopilot SKILL, task-list-canonical, post-implementation, gate-validation, phase-execution, and agent-teams-integration surfaces — changed as needed to show one 13-row list and call the new rule immediately before successful completion.
- `speckit-pro/skills/speckit-coach/templates/workflow-template.md` — changed: 13 Post rows.
- Four Claude executor Markdown files and four Codex executor TOML files for phase, analyze, checklist, and implement — changed: collect child result or supported stop, request graceful shutdown, verify no active child and completed cleanup, identify unresolved confirmation in result.
...
These ten named operations are the currently identified source/document subset of Slice A's provisional 16-file total. The other six projected paths (fixtures, generated output, or support documentation) are not named yet, so this is **not** a complete `Declared File Operations` block and must not be used to claim a valid estimator result or a passing file budget. The plan-phase reviewability estimator should report `not_estimated` until Tasks supplies a complete per-slice operation list. Later slices likewise require their own complete inventories before a PR; combining four slices into one operation block would hide the split decision.
...
**Before Phase 0 — PASS with a required split.** Existing plugin structure and manifest versioning remain intact; repository Python stays standard-library and no active Bash/jq is added; new deterministic helper behavior receives Layer 4 fixtures and host components Layer 1/5 coverage; conventional commit/PR-title gates apply at delivery; no new abstraction or plugin is introduced. The whole feature exceeds the reviewability block, so the four ordered slices are mandatory rather than an exception. B and C2 exceed the 400-LOC warning but remain below the 800-LOC block. Typed `Reviewability-Exception` classes remain `refactor`, `infra`, and `upgrade`, with no automatic split exception.

**After Phase 1 design — conditional on the Tasks sizing checkpoint and per-slice diff gate.** The data model and contracts below preserve existing request/result envelopes and host parity. The API pagination behavior and explicit operator signal are pinned in [research.md](research.md); Codex post-parent child lifetime is deliberately outside scope and does not weaken the teardown result contract. The refactor-inclusive size estimate remains explicitly `not_estimated` on the installed helper; Tasks must close that evidence gap before treating the four-slice budget as qualified. Before each PR, count actual authored and generated changed paths and reviewable LOC; if any slice reaches 25 total files, exceeds four production files, or crosses a block threshold, split/rescope before implementation completion.
...
3. **C1 — Post and teardown.** Freeze 13-row template/host mismatch and persisted workflow/state cases with missing, duplicate, pending, and in-progress rows. Build the new completion rule from `POST_STEPS` and call it on both hosts immediately before successful return. Existing `status-evidence` remains a separate audit and cannot prove Post completion. Freeze child result/stop and teardown evidence for all eight executor definitions; their clean completion requires confirmed cleanup. Review formal lifecycle's Post subset.
