...
- **FR-014**: On acceptance, scaffold MUST invoke the autopilot in-session with the workflow file path and `--stage plan` (Q4). The stage token MUST be the literal lowercase `plan` from the ART-006 contract §3 closed vocabulary of `plan`, `implement`, `full` — no aliases, no alternate casing, no long-form spellings. The workflow file path MUST be the sole hand-off token; scaffold MUST NOT pass a state file, branch name, feature directory, or environment variable across the boundary (ART-006 §1). The runnable forms are:
  ...
  **The hand-off MUST be visible at the moment it happens.** On acceptance, scaffold MUST print the invocation from the table above, verbatim, immediately before running it. Without that line the accepted path is the only branch point in the run where the operator is told nothing about what just happened: the next thing they see is another skill's output, arriving with no statement that the confirmation took effect, that the run is now autonomous, or that what follows belongs to the planning stage rather than to scaffold. ART-006's own framing calls this boundary a visible seam, and one printed line is what makes it one. It also puts the same string in front of the operator on both branches — as what is now running when they accept, and as what to run later when they decline — so the two paths teach the same command instead of two.
...
- **FR-016**: The existing "Scaffold Complete" report MUST still print before the confirmation, so the operator is told what scaffold produced before being asked whether to continue (Q5). The two variants place that existing report differently, so the insertion point differs even though the requirement does not: the Claude report sits inside Step 7, ahead of Step 8, so the chain and closing report append after the end of its procedure; the Codex report is a top-level Output section that already follows Step 8, so the chain and closing report extend that section.
- **FR-017**: A single closing report MUST render on **every** terminal condition the run can reach — after the planning stage on acceptance, and immediately on each of the three FR-015 no-chain paths: the operator declined, no structured confirmation mechanism was available, or the FR-013a pre-chain check failed (Q5). It MUST be printed, not written to a file.
...
- **FR-018**: The closing report MUST contain, in this order: the run outcome; a draft pull request line; an artifact index; and one next step. Its layout is:
  ...
  **The heading is a closed three-value vocabulary, one per terminal condition**, because US4's own independent test drives the run to three and a two-value vocabulary would force one of them to be reported under a heading that is false:

  | Terminal condition | Heading |
  |---|---|
  | The operator declined, or the chain never fired | `## Stopped Before Planning` |
  | The chain fired and the FR-019 completion test passes | `## Planning Complete` |
  | The chain fired and the FR-019 completion test does not pass — failed, stalled, or interrupted | `## Planning Incomplete` |
...
- **FR-022**: Both platform variants MUST implement the same flow, differing only where the platform forces a difference: the invocation form, the confirmation mechanism name, and the local documentation idiom. The production surface MUST be exactly two files, `speckit-pro/skills/speckit-scaffold-spec/SKILL.md` and `speckit-pro/codex-skills/speckit-scaffold-spec/SKILL.md`, and neither may gain a `references/` directory. The Codex `agents/openai.yaml` sidecar is deliberately excluded from that surface: it carries no `description` field, so FR-021 does not reach it, and its `default_prompt` is pinned verbatim by `tests/speckit-pro/layer1-structural/validate-codex-skills.py`, so changing it would cost a third production file plus a test edit.
  ...
  **Permitted divergences, now four.** The invocation form; the confirmation mechanism name; **whether the chain is attempted at all**, per FR-015a and FR-015b; and the local documentation idiom. The third is new to this spec and is the reason this requirement carried the last open marker: it is a platform difference the design concept's Q4 did not anticipate, and it is forced by shipped Codex behaviour rather than chosen.
...
- **SC-007**: This feature adds **at most one** operator confirmation to a scaffold run, and exactly one whenever the chain is attempted. It adds none when the FR-013a pre-chain check fails, which on Codex CLI is the ordinary case. *(AC-11.2)*
  ...
  **This is a cap, not headroom.** "At most one" bounds what this feature may introduce and MUST NOT be read as permission to add a second prompt elsewhere: FR-013 introduces the only one, FR-011 forbids one at the findings stage, FR-018's `## Planning Complete` rule forbids one to offer the implement stage, and the informational lines FR-013 and FR-014 require are printed statements carrying no options, which is why they do not count. The design concept's Q16 rationale says scaffold sits at "exactly one confirmation — the chain"; that decision is untouched, and only its unexamined premise about the pre-existing prompts is corrected here, in the same manner as the dated Q4 premise correction in its Revision Notes.
...
- **SC-011**: A reader comparing the two platform variants **across the flow this feature specifies** — the blind-spot pass, the seeding and its record, the chain hand-off, and the closing report — finds no behavioural difference between them other than the invocation form, the confirmation mechanism name, whether the chain is attempted at all, and the local documentation idiom. Every operator-facing string this feature fixes is identical on both platforms; the four permitted divergences are structural, not textual. *(AC-11.4)*
