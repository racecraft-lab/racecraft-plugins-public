---
# Spec-level Map of Content (MOC). Frontmatter join-key contract (PRSG-002).
# Consumed by speckit-scaffold-spec via token substitution (same token mechanism
# as workflow-template.md).
#
# up: MUST be a quoted relative markdown link, NEVER a wikilink. The target is
# load-bearing: stale-index lint resolves it from this spec directory.
up: "[HTML artifacts and staged review roadmap](../../docs/ai/specs/html-artifacts-technical-roadmap.md)"
related: []                  # list of relative markdown links; carried, unenforced in v1
status: "In Progress"        # carried, unenforced in v1
rank:                        # carried, unenforced in v1
spec_id: "ART-011"           # namespace-matches the containing directory
structureVersion: 1          # keep in sync with the lint scripts' hardcoded literal
---

# ART-011 — Map of Content

Navigation map for this spec. The `up:` link above points to the parent technical
roadmap. As this spec is decomposed into slices, add relative Markdown links to
the slice artifacts in the body below.

<!-- GENERATED:INDEX:START (do not edit; regenerated by generate-spec-index) -->
<!-- GENERATED:INDEX:END -->

<!-- GENERATED:PRS:START (do not edit; regenerated by generate-spec-index) -->
<!-- GENERATED:PRS:END -->

<!-- GENERATED:BACKLINKS:START (do not edit; regenerated by generate-spec-index) -->
- [spec.md](spec.md)
- [plan.md](plan.md)
- [tasks.md](tasks.md)
- [research.md](research.md)
- [contracts/blind-spot-pass.md](contracts/blind-spot-pass.md)
- [contracts/chain-handoff.md](contracts/chain-handoff.md)
- [checklists/api-contracts.md](checklists/api-contracts.md)
- [checklists/error-handling.md](checklists/error-handling.md)
- [checklists/requirements.md](checklists/requirements.md)
- [checklists/ux.md](checklists/ux.md)
- [.process/changed-files.txt](.process/changed-files.txt)
- [.process/pr-body.md](.process/pr-body.md)
<!-- GENERATED:BACKLINKS:END -->
