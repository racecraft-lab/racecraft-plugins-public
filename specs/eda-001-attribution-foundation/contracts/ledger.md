...
The test exits nonzero on each defect. A missing ledger file or unparseable ledger JSON fails before row checks and names the repository-relative ledger filename plus the missing-file or JSON-parse defect. There is no row identity before successful row parsing; omit unavailable row context rather than inventing it. Isolated negative cases remove the valid ledger file or replace its bytes with unparseable JSON, then assert the input-specific diagnostic. A schema error names the input file and row index/path where available, plus the field. Inventory errors identify missing/extra/duplicate/out-of-order paths; byte-format errors identify the ledger. No malformed input is silently coerced, skipped, or rewritten by validation. Negative tests assert the targeted diagnostic, not merely an arbitrary exception.
