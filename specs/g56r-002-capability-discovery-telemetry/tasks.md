...
modules, 10 implementation files, and one primary surface. Stop and decompose if
...
- [ ] T005 [US1] [FR-004] Add RED tests in `tests/speckit-pro/unit/test-g56r-002-capability-telemetry.py` for one canary per snapshot/model/effort, an injected fake `CanaryExecutor`, the closed result envelope, default-empty repository approval allowlist, required contract/implementation/result digests, timeout/output-cap/process-tree acknowledgements, zero retry, arbitrary self-approval rejection, every terminal class, exit-zero sentinel success, raw-root containment/permissions, and deny-by-default sanitization; Acceptance: the tests launch no process and fail only on missing adapter behavior, not malformed fixtures.
...
- [ ] T007 [US1] [FR-001] [FR-002] Implement canonical JSON/SHA-256, current-ledger and effort-surface selection/validation, punctuation-only/undocumented effort non-admission, allowlisted source-refresh records, and `client_identity_id` validation in `tests/speckit-pro/layer6-efficiency/lib/codex_capabilities.py`; Acceptance: T003 tests pass while T004–T006 remain the only RED cases.
...
- [ ] T009 [US1] [FR-004] Implement the injected `CanaryExecutor` closed result contract, versioned default-empty repository approval allowlist, fail-closed missing/unapproved/self-asserted executor paths, closed terminal taxonomy, external raw-store boundary, permission checks, deterministic pseudonymization, allowlist sanitizer, and canonical sanitized fixture output in `tests/speckit-pro/layer6-efficiency/lib/codex_capabilities.py`; Acceptance: T005 passes, only an injected test allowlist can admit the fake executor in this slice, and no default path launches a process or network request.
- [ ] T010 [US1] [FR-003] [FR-004] Implement runtime snapshot, executable tuple, append-only freeze, successor, and narrow `refresh-sources`, `identify-client`, `collect`, `canary --executor-result`, and `freeze` command dispatch in `tests/speckit-pro/layer6-efficiency/lib/codex_capabilities.py`; Acceptance: T006 passes, a missing/unapproved executor result fails closed, and unsupported arguments fail nonzero without shell execution.
...
- [ ] T016 [US2] [FR-005] [FR-006] Create sanitized telemetry-profile, configured-route, controlled-environment, six-ID objective, route-resolution, destination-assessment, structured treatment-failure, and exact-treatment records in `tests/speckit-pro/unit/fixtures/capability-treatment-replay/treatment-replay.json`; Acceptance: `python3 -m json.tool tests/speckit-pro/unit/fixtures/capability-treatment-replay/treatment-replay.json` passes and every closed-inventory field has source/class/completeness/claim/observation-rule metadata.
...
- [ ] T018 [US2] [FR-002] [FR-006] Add RED tests for all six non-null objective IDs, the controlled-environment owner registry and client/surface/repository/candidate/work-item equality, duplicate owning IDs, preferred/attempted/assigned/effective route fields, agent/model/effort/configuration hashes, sandbox/approvals/mutation class, expected/loaded skills/MCP/tools, parent configuration, client/overrides, delivery canary, context, and parent-child graph in `tests/speckit-pro/unit/test-g56r-002-capability-telemetry.py`; Acceptance: a missing/duplicate environment owner or mismatched binding deterministically fails while repeated foreign-key references remain valid.
- [ ] T019 [US2] [FR-006] [FR-007] Add RED tests for structured treatment failures, raw token vector, request/turn count, wall time, retries, compaction, validation, cancellation, failed/abandoned work, terminal state, outcome/acceptance, raw reroute events, separate destination route/agent/prequalification assessments, ambiguous joins, same-agent prequalified continuation, and non-scorable/hard-fail dispositions in `tests/speckit-pro/unit/test-g56r-002-capability-telemetry.py`; Acceptance: each failure class has one explicit expected result, missing/mismatched destination proof hard-fails, and resolver fields never change after a service reroute.

### GREEN — Neutral Schema Validator

- [ ] T020 [US2] [FR-005] Implement telemetry-profile entry validation, class/claim semantics, closed inventory, profile-key uniqueness, required observation-state rules, and typed value/null/evidence validation in `tests/speckit-pro/layer6-efficiency/lib/treatment_trace_schema.py`; Acceptance: T017 passes with missing/native fields never inferred.
...
- [ ] T038 [US1] [FR-003] [FR-008] Re-run the reviewability gate against `docs/ai/specs/.process/G56R-002-workflow.md` and inspect implementation LOC/files; Acceptance: one primary surface remains, production modules remain at two, total implementation files remain at ten, and reviewable LOC stays below 400.
