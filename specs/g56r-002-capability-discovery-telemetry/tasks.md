...
- [x] T005 [US1] [FR-004] Add RED tests in `tests/speckit-pro/unit/test-codex-capability-contract.py` for one canary per snapshot/model/effort, an injected fake `CanaryExecutor`, the closed result envelope, default-empty repository approval allowlist, required contract/implementation/result digests, timeout/output-cap/process-tree acknowledgements, zero retry, arbitrary self-approval rejection, every terminal class, exit-zero sentinel success, raw-root containment/permissions, and deny-by-default sanitization; Acceptance: the tests launch no process and fail only on missing adapter behavior, not malformed fixtures.
...
- [x] T031 [US1] [FR-001] [FR-008] Register `tests/speckit-pro/unit/test-codex-capability-contract.py` in `tests/speckit-pro/suite-manifest.json` with a unique label and deterministic default-layer execution; Acceptance: suite-manifest validation and the focused test both pass.
