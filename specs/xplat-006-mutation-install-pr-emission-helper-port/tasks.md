...
- [ ] T035 [US2] Record Slice 2 promotion evidence in `tests/speckit-pro/layer4-scripts/fixtures/mutation-helpers/promotion-records.json`.
- [ ] T036 [HANDOFF] Update runner manifest/checksum metadata after install helper and inventory changes.
- [ ] T037 [US2] Run the install-doctor focused test group and record evidence in the PR packet section of `docs/ai/specs/.process/XPLAT-006-workflow.md`.
...
- [ ] T038 [P] [US3] Add failing PR-emission fixture tests for `generate-pr-body`, `generate-uat-skeleton`, `final-reviewability-backstop`, PR-packet output, and workflow-contract output.
- [ ] T039 [P] [US3] Add failing fake `gh` and fake repo fixture tests for `multi-pr-emission`, `restack`, split-PR state, migration, and relocation apply boundaries.
- [ ] T040 [P] [US3] Add failing generated-output fixture tests for generated-index write/regenerate modes, `plan-layers` marker-plan output, `validate-pr-packet` workflow-event persistence, and `validate-pr-workflow-contract` write mode.
- [ ] T041 [US3] Add `speckit-pro/speckit_pro_runner/helpers/pr_emission.py` with atomic generated artifact writes and preserved packet/body content contracts.
- [ ] T042 [US3] Port PR-body, UAT-skeleton, final-reviewability-backstop, PR-packet, and workflow-contract output helpers into `helpers/pr_emission.py`.
- [ ] T043 [US3] Port `multi-pr-emission`, `restack`, split-PR state, migration, and relocation helper behavior using fake repos and fake `gh` by default.
- [ ] T044 [US3] Port generated-index write/regenerate modes, `plan-layers` marker-plan output, `validate-pr-packet` persistence/workflow-event upserts, and `validate-pr-workflow-contract` write mode.
- [ ] T045 [US3] Keep `detect-stack-manager` to decision and command-plan evidence only; verify mutating command execution remains owned by `multi-pr-emission` and `restack` apply paths.
- [ ] T046 [US3] Record Slice 3 promotion evidence in `tests/speckit-pro/layer4-scripts/fixtures/mutation-helpers/promotion-records.json`.
- [ ] T047 [HANDOFF] Update runner manifest/checksum metadata after PR-emission helper changes.
- [ ] T048 [US3] Run the PR-emission focused test group and record evidence in the PR packet section of `docs/ai/specs/.process/XPLAT-006-workflow.md`.
...
- [ ] T049 [HANDOFF] Run `python3 tests/speckit-pro/layer4-scripts/test-autopilot-phase-coverage.py` and record 6/6 hardening result.
- [ ] T050 [HANDOFF] Run `python3 speckit-pro/skills/speckit-autopilot/scripts/validate-autopilot-phase-coverage.py --workflow docs/ai/specs/.process/XPLAT-006-workflow.md --state docs/ai/specs/.process/autopilot-state.json` and record `status=pass`.
- [ ] T051 [HANDOFF] Run `python3 tests/speckit-pro/layer4-scripts/test-speckit-pro-runner.py` and record the scope-audit test result.
- [ ] T052 [HANDOFF] Run `python3 tests/speckit-pro/layer4-scripts/test-speckit-pro-mutation-helpers.py` and record promoted helper coverage.
- [ ] T053 [HANDOFF] Run Bash-reference comparison fixtures from `bash-reference-manifest.json` and record comparison ids for every Bash-backed promoted helper.
- [ ] T054 [HANDOFF] Run `bash tests/speckit-pro/run-all.sh --layer 4` and record Layer 4 totals.
- [ ] T055 [HANDOFF] Run `bash tests/speckit-pro/run-all.sh --layer 1` and record Layer 1 totals.
- [ ] T056 [HANDOFF] Run `bash tests/speckit-pro/run-all.sh` and record default deterministic suite totals.
- [ ] T057 [HANDOFF] Run `bash speckit-pro/skills/speckit-autopilot/scripts/generate-spec-index.sh --check $PWD` and `bash tests/speckit-pro/layer1-structural/validate-moc-stale-index.sh specs`.
- [ ] T058 [HANDOFF] Run `git diff --name-only origin/main...HEAD` and scope-audit the diff for forbidden active cutover surfaces, listing the allowed hardening source/mirror separately.
- [ ] T059 [HANDOFF] Update `docs/ai/specs/.process/XPLAT-006-workflow.md` with verification evidence, known gaps, approval boundaries, rollback/manual remediation notes, and XPLAT-007/XPLAT-008 handoff.
- [ ] T060 [HANDOFF] Ensure the PR packet states that active Claude/Codex invocation cutover, generated-payload selection/cutover, repo-local release-gate migration, native matrix UAT, and public native-platform support claims are not delivered by XPLAT-006.
...
- US2 tasks T028-T037 depend on T007-T019 and may run after US1 mutation primitives are stable.
- US3 tasks T038-T048 depend on T007-T019 and may run after US1 mutation primitives are stable.
- Cross-cutting verification T049-T060 depends on all promoted helper tasks and promotion records.
...
- T038-T040 can run in parallel because they add independent US3 fixture groups.
