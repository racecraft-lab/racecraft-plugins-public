...
- **FR-006a**: The artifacts MUST record which inputs can induce the FR-006 skip,
  ...
  unrecorded way to disable the check. The audit found three such inputs, of
  which FR-006b closes the second, leaving two live. None of the three is the
  state's `workflow_file` value:
...
budget, so it needs no operator override. Four of the ten touched files are
...
- **Split decision**: Remains one specification. The slice estimator returned
  `estimated_loc 337`, `suggested_slices 1`, `status ok` against signals of 3 user
  stories, 6 authored files, 24 functional requirements, modify-weighted. That sits
  under the 400 reviewable-LOC ceiling, and production files at 5 sit under the
  ...
  split would produce fragments that cannot be verified on their own.
- **Revision note, Tasks phase**: this block previously declared 235 reviewable
  LOC across 4 production files and 9 total, against estimator signals of 5
  authored files and 13 functional requirements. Both inputs grew after that
  declaration was written. Clarify and the three checklist domains expanded the
  requirement set from 13 to 24, and Tasks found that FR-013c's "on both
  platforms" obligation requires amending the Codex references index, which lives
  in a sixth authored file the plan had not declared. The estimator was re-run on
  the corrected signals and returned 337. Every verdict is unchanged: still one
  slice, still within budget, still no typed exception. The figures above are the
  re-run ones. This also revises, in the same direction, the roadmap's earlier
  declaration of roughly 120 reviewable LOC across 2 production files and the
  design concept's Q12 amendment to 235 across 5 authored files, neither of which
  could anticipate the requirement growth; Q12 is recorded there as an advisory
  note rather than a decision, so raising the figure contradicts no settled
  scoping choice.
...
- The recorded corpus evidence MUST be reproducible from the workflow file alone.
  ...
  fact that writing it inside the repository was necessary but **not sufficient**
  when the before-half was measured, because the root walk then read the state
  path as passed, so the harness MUST also pass it in a form the walk can resolve
  from, meaning an absolute path or a relative one with the working directory at
  the repository root, and MUST record which. FR-006b removes that dependence in
  the repaired guard, which resolves the state path before walking it, so the
  after-half no longer relies on the spelling. The requirement stands for both
  halves anyway, for two reasons: the before-half is already banked under it and
  reusing the same harness is what makes the pair comparable, and recording the
  form used is what lets a later reader tell a genuine after-half pass from one
  that would have passed under the old walk too. The evidence MUST also record
  the exact guard invocation; the before and after counts; and one deliberately
