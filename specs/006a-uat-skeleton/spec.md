...
- **Unresolved clarification markers in the source spec**: when `spec.md` still carries clarification markers, the runbook propagates each one with an unresolved-clarification annotation rather than dropping it silently (FR-005).
...
- **FR-004**: When `spec.md` has duplicate FR/SC IDs (for example, FR-005 appearing twice as in SPEC-004), the script MUST emit the first-seen entry only and write a stderr warning naming the duplicate ID.
...
- **FR-006**: The script MUST emit exit code 0 on success, 2 on usage error (wrong argv), and 1 on an unreadable spec.
...
- **FR-008**: The script MUST accept the autopilot's project commands via the environment variable `UAT_PROJECT_COMMANDS` (a JSON string). When unset, the Env Setup section MUST emit `<unknown — autopilot did not pass PROJECT_COMMANDS>` placeholders rather than failing.
...
- **FR-013**: `generate-pr-body.sh` MUST add `"UAT Runbook"` to its heading list AND add a corresponding section to the review-packet body that reads `<feature-dir>/uat-runbook.md`. When the runbook is under 50,000 characters, it MUST embed the full content via `extract_heading_section`; otherwise it MUST embed an opening excerpt and append a relative link. [NEEDS CLARIFICATION: the exact "opening excerpt" definition for the over-threshold path — first N markdown lines, first N non-blank lines, or up to the first User Story section boundary. Deferred to Clarify Session 2 (template rendering edge cases).]
...
- **Projected total files**: 9 (the 4 production files, the vendored fixture, plus lockstep Codex-variant edits and autopilot reference updates).
...
- **Reviewability budget**: ~670 LOC, 4 production files (template, script, modified `generate-pr-body.sh`, Layer 4 test), 9 total files — under the block thresholds.
