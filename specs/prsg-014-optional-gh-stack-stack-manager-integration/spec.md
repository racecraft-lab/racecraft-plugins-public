...
- **FR-002**: Autopilot MUST record a deterministic `stack_manager_decision` object containing schema version, selected manager, reason, fallback reason when applicable, mutation boundary, `gh_stack` availability/support/version/invocation details, repository compatibility, topology compatibility, read-only proof status, command plan, and PR/branch topology.
