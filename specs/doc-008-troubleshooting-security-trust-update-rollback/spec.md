...
- Browser-rendered docs must not run a local doctor command, local filesystem probe, permission request, or automatic repair.
...
- **FR-003**: The troubleshooting page MUST replace the DOC-002 shell with a symptom-driven matrix whose rows include symptom, likely cause, diagnostic command or file, recommended fix, platform label, and follow-up link.
- **FR-004**: Troubleshooting coverage MUST include failed installs, stale plugin behavior, stale marketplace or payload state, missing Codex custom agents, permission prompts, path confusion, version drift, missing or outdated Spec Kit CLI prerequisites, and source-vs-generated-payload mismatch.
...
- **FR-006**: Browser docs MUST NOT execute local commands, request local permissions, inspect the local filesystem, or modify user configuration; all diagnostics must be presented as user-run commands, files to inspect, or source links.
...
- **FR-008**: Platform-behavior claims MUST cite current official vendor docs verified during implementation, including Claude Code plugin, settings, permissions, sandbox/security, hooks, subagent, managed marketplace, and marketplace docs, plus OpenAI Codex plugin, skills, subagents, sandboxing, permissions, approvals/security, managed configuration, and AGENTS.md docs.
...
- **FR-011**: The update and rollback page MUST define and distinguish update, refresh, reinstall, remove, rollback, stale payload, stale cache, and version-sync cases for Claude Code, Codex, or both.
- **FR-012**: Recovery guidance MUST tell users to refresh marketplace sources, update or regenerate source-owned payloads through the documented project flow, reinstall or reload/restart through the platform, and avoid direct edits to installed plugin caches or generated payloads.
- **FR-013**: Codex recovery guidance MUST preserve the separation between plugin installation, bundled skill loading, and custom-agent registration through `@SpecKit Pro -> install` or `$install`.
- **FR-014**: Claude Code recovery guidance MUST preserve the separation between marketplace update, plugin install/uninstall, `/reload-plugins`, plugin detail inspection, managed policy, and installed runtime state.
- **FR-015**: The three pages MUST cross-link to the platform install guides, DOC-007 reference pages, first-run/lifecycle docs where relevant, and each other for follow-up from symptom to trust or recovery detail.
- **FR-016**: DOC-008 MUST NOT change plugin behavior, generated payload semantics, manifests, hooks, release automation, CI behavior, SpecKit CLI behavior, browser-side diagnostics, or local repair commands.
- **FR-017**: DOC-008 validation MUST use the docs-site verification bundle: `pnpm --dir docs-site validate`, `pnpm --dir docs-site validate:links`, `pnpm --dir docs-site reference:check`, and the configured full verification command.

### Source Evidence Confirmed For Implementation

- Claude Code official docs: [Discover and install prebuilt plugins](https://code.claude.com/docs/en/discover-plugins), [Create plugins](https://code.claude.com/docs/en/plugins), [Settings](https://code.claude.com/docs/en/settings), [Permissions](https://code.claude.com/docs/en/permissions), [Sandbox environments](https://code.claude.com/docs/en/sandbox-environments), [Security](https://code.claude.com/docs/en/security), [Hooks](https://code.claude.com/docs/en/hooks), and [Subagents](https://code.claude.com/docs/en/sub-agents).
- OpenAI Codex official docs: [Plugins](https://developers.openai.com/codex/plugins), [Build plugins](https://developers.openai.com/codex/plugins/build), [Agent Skills](https://developers.openai.com/codex/skills), [Subagents](https://developers.openai.com/codex/subagents), [Sandbox](https://developers.openai.com/codex/concepts/sandboxing), [Permissions](https://developers.openai.com/codex/permissions), [Agent approvals & security](https://developers.openai.com/codex/agent-approvals-security), [Managed configuration](https://developers.openai.com/codex/enterprise/managed-configuration), and [AGENTS.md](https://developers.openai.com/codex/guides/agents-md).
...
- **Projected total files**: About 6 docs/process files, including three user-facing pages plus navigation/linking or generated-reference touch points if existing docs-site conventions require them
...
- **Troubleshooting Row**: A symptom-driven matrix row with symptom, likely cause, diagnostic command or file, recommended fix, platform label, and follow-up link.
- **Platform Label**: The row-level label `Claude Code`, `Codex`, or `Both` used to show where a symptom or procedure applies.
- **Trust Claim**: A statement about platform behavior, repository fact, or recommended practice that must be grouped by evidence type and backed by a source.
...
- **Recommended Practice**: User guidance derived from official behavior and repository facts, such as not editing installed caches or generated payloads directly.
- **Recovery Case**: One of update, refresh, reinstall, remove, rollback, stale payload, stale cache, or version sync.
...
- **SC-002**: The troubleshooting matrix includes at least one row for each required failure class in FR-004, and 100% of rows include all six required row fields.
...
- **SC-006**: The update and rollback page defines all eight recovery cases from FR-011 and gives at least one safe Claude Code or Codex recovery path for each applicable case.
