...
- **FR-023**: The system MUST use calibration and historical non-release evidence only to publish a plan-free calibration-completion artifact and freeze the numeric analysis plan before G56R-007 through G56R-010 observe outcomes.
...
- **FR-038**: Before calibration, one schema-governed, versioned calibration protocol MUST freeze the calibration partition and operational authority bindings without margins, sample sizes, quality floors, or terminal thresholds. After calibration, one content-addressed calibration-completion artifact MUST bind that protocol, the ineligible partition, comparison sets, assignments, score bundles, evidence, and independent review; MUST prove calibration execution is complete before any analysis plan or later-cohort outcome is observed; and MUST contain no margins, sample sizes, quality floors, or terminal thresholds. Before any G56R-007 through G56R-010 cohort outcome is observed, one schema-governed, versioned analysis plan MUST bind both the protocol and completion artifact and freeze workload strata, p95 guardrails, margins, sample sizes, sample-size assumptions, power, alpha, multiplicity, racing and futility rules, attrition caps, campaign budgets, cache policy, and terminal rules.
- **FR-039** *(AC-2.2, AC-2.3, AC-2.19)*: The system MUST detect an alias re-point from a mismatch between the run-observed model identity and the resolved qualified identity for the requested alias, record the event as bounded platform attribution, and never report it as SpecKit Pro fallback. Detection MUST bind five observables: requested alias, resolved identity from G56R-003's published successor freeze, run-observed identity from the run's supported model-identity evidence, the complete controlled-runtime-override proof frozen by the environment contract, and pinned client version at freeze and run time. Platform attribution is permitted only when the requested route and client are unchanged and every controlled override is proven absent; plugin substitution is resolver fallback, and an incomplete or otherwise unattributable divergence MUST remain unresolved and block scoring.
- **FR-040** *(AC-2.1, AC-2.11)*: Capability discovery MUST probe and record each role-eligible model's complete ordered supported ordinary-effort set and actual documented default, and the within-model search MUST start at that model's default before ascending, descending, and boundary-retesting. G56R-003 MUST NOT impose one Claude-derived `low`-through-`max` ladder on every Codex model: the pinned Codex 0.145.0 capture records `gpt-5.3-codex-spark` and `gpt-5.5` through `xhigh`, `gpt-5.6-luna` through `max`, and `gpt-5.6-sol` and `gpt-5.6-terra` through `max`, with their respective defaults. Ultra is recorded separately as a topology-changing control and MUST NOT enter an ordinary candidate-effort tuple.
- **FR-041** *(AC-2.2, AC-2.12)*: Versioned refresh triggers MUST cover client change, catalog change, alias re-point, and source-ledger change and declare what each invalidates and preserves. Each trigger invalidates freeze admission, unexecuted derived bindings, and affected experiment, score, and decision bundles additively; immutable execution traces, treatment records, and already-bound pairs survive unchanged but are marked invalidated. Alias re-point additionally makes in-flight attempts for that alias non-scorable, and source-ledger change alone cannot admit a runtime-unsupported tuple.
- **FR-042** *(AC-2.19, amended 2026-07-26)*: ChatGPT sign-in is the supported scored baseline. API-key sign-in remains a valid Codex mode only when a separate pre-execution environment contract pins it; it is not required for the baseline and MUST NOT be substituted silently. Every run MUST record and compare its observed authentication mode with the frozen contract, and mismatch blocks scoring. Authentication MUST NOT produce plan, entitlement, billing, or route-preference claims.
- **FR-043** *(AC-2.19)*: The dual-platform smoke runner is jointly owned by the G56R and CAR evaluation branches. Changes to that shared surface MUST be coordinated from the current default branch and overlap MUST be integrated explicitly rather than rewritten unilaterally; neither platform may silently change the other's treatment or measurement contract.
- **FR-044** *(AC-2.2, AC-2.12)*: An empty or invalid source/runtime intersection MUST emit diagnostic collection evidence only, publish no authoritative successor freeze, block qualification-capable execution, and never reuse, rewrite, or promote predecessor tuples as an active set. Every admitted tuple's runtime evidence MUST resolve to the collection named by its own successor freeze; predecessor evidence is provenance only.
- **FR-045** *(AC-2.3, AC-2.19)*: Alias-re-point attribution MUST live in a new additive G56R-003 record rather than extending any frozen closed enum. The claim is bounded to observed identity divergence over the enumerated causes; behavioral divergence without an identity change is a separate diagnostic condition and MUST NOT be labeled a re-point.
- **FR-046** *(independently adopted; no direct AC-2.x anchor)*: The alias-re-point detector MUST be covered by a committed synthetic replay fixture with a known observed-versus-resolved identity divergence and genuinely absent controlled overrides. If that replay cannot be built, coverage MUST be reported as unvalidated in band rather than claimed.
- **FR-047** *(independently adopted validity safeguard adjacent to AC-2.20; no direct AC-2.x mandate)*: A scorer or adjudicator MUST NOT come from the same model family as a candidate it scores. The frozen exclusion MUST be checked against each scorer's or adjudicator's run-observed identity, not merely its declared route; route divergence blocks the ballot, and a same-family ballot MUST NOT count. Presentation order remains seeded, and no non-frozen paraphrasing or style-normalization model call may alter the artifact being scored.
- **FR-048** *(AC-2.20)*: Blinding MUST be reported as bounded, never complete. Each ballot MUST record whether the scorer inferred candidate provenance and every signal reported for that inference. Inference does not silently invalidate a ballot, but the complete residual MUST accompany qualification reporting.
- **FR-049** *(AC-2.3, AC-2.5)*: `reasoning_output_tokens` MUST be recorded and reported for every attempt but MUST NOT become a ninth decision-bearing Pareto dimension without a joint cross-platform contract change. The omission is a stated limitation, not a claim that reasoning cost is absent. Each arm MUST also record the observed cache root actually used; only pairs with distinct observed roots and `status=observed_disjoint` may contribute to resource comparison.
- **FR-050** *(AC-2.12, AC-2.16)*: The frozen multiplicity declaration MUST address three families separately: the conjunctive floors and non-inferiority ladder, the Pareto stage's disjunctive better-on-at-least-one half, and comparisons across candidate, role, and stratum ladders. It MUST state the rule and rationale for each and record cluster-adjusted variance as a precondition, not as a multiplicity correction.
- **FR-051** *(AC-2.6, AC-2.19)*: Before execution, every attempt MUST bind a versioned environment contract covering a permitted client-version range, parent-session model and effort, the complete controlled-runtime-override proof, authentication mode, and `ultra_state=off`. Ultra-off is an admission precondition, never an instruction to mutate the operator's setting: no SpecKit Pro surface may enable or disable Ultra. The harness observes and refuses. A confirmed contract divergence is non-scorable with existing `failure_plane=treatment` and `failure_code=treatment_infrastructure_failure`; an unobservable environment uses existing `failure_plane=evidence_boundary` and `failure_code=required_evidence_missing`. The gate runs uniformly before outcomes are read and reports exclusion counts.
- **FR-052** *(AC-2.8, AC-2.9)*: Workload-stratum membership MUST be bound before either arm runs and derived from a non-empty subset of the already-bound pre-execution characteristics `role_id`, objective, permitted tools, mutation contract, expected artifacts, and acceptance oracle. `derived_from_realized_outcomes` MUST be false; duration, turns, tokens, retries, compactions, and other realized outcomes cannot define membership. Each long-horizon stratum carries its own sample size and minimum unique-task count, unknown strata return inconclusive, and frozen strata cannot be redrawn after an eligible outcome is observed.
- **FR-053** *(AC-2.15)*: Every frozen p95 resource and duration guardrail MUST declare guarded quantity and unit, denominator, absolute-or-relative comparator, a guardrail-specific margin, confidence method, missing-data rule, worse direction, and multiplicity position. Silent exclusion of missing upper-tail attempts is prohibited. Guardrails form a distinct family, remain outside the eight Pareto dimensions, and a breach returns no qualification without weighting or tradeoff.
- **FR-054** *(AC-2.8, AC-2.9, AC-2.15)*: Each stratum MUST freeze its own sample size and minimum unique-task count. A guardrail is admissible only when that stratum meets its own floor; a pooled manifest floor cannot substitute. A shortfall is reported and returns inconclusive rather than pass or skip.
- **FR-055** *(AC-2.11, AC-2.16)*: Frozen racing and futility declarations MUST state every interim look and information fraction, stopping boundary, error control, complete-pair stop scope, and, for futility, whether the boundary is binding. Even a disabled policy MUST explicitly declare zero looks. A look added, moved, or repeated after an outcome invalidates the declaration, and an early stop is reported as stopped rather than completed.
- **FR-056** *(AC-2.7, AC-2.11, AC-2.16)*: Campaign-ceiling exhaustion is distinct from candidate budget exhaustion. Candidate exhaustion remains in the estimand at acceptance zero with `candidate_budget_exhausted`; campaign exhaustion between arms is an incomplete administrative truncation recorded with existing `failure_plane=infrastructure` and `failure_code=infrastructure_failure`, returns inconclusive, and cannot receive a one-arm rerun. After outcomes are observed, a ceiling or guardrail cannot be relaxed in place; a changed plan receives a new identity and its outcomes cannot be pooled with the superseded plan.
- **FR-057** *(independently adopted operability safeguard; no direct AC-2.x anchor)*: The default deterministic suite MUST make zero live model calls, stay within a declared wall-clock budget derived from a measured baseline, keep replay fixtures bounded rather than growing per campaign, and preserve the existing p95 frozen-bundle replay target under ten seconds. Work that exceeds the default-suite budget moves behind the explicit operator-only live path.
- **FR-058** *(AC-2.5, AC-2.16, AC-2.21)*: Each of the eight decision-bearing dimensions has fixed comparison semantics: raw input tokens, cached-input tokens, output tokens, duration, retries, and compactions are lower-is-better; acceptance is higher-is-better; terminal state is categorical and no-worse only when equal. A terminal-state difference makes the result mixed. These directions define comparison behavior without adding a ninth dimension or changing the closed dimension inventory.
...
- **Calibration Completion**: Content-addressed, plan-free proof that the protocol-bound calibration execution and its comparison, assignment, score, evidence, and independent-review bindings are complete before an analysis plan or later-cohort outcome exists; it carries no margins, sample sizes, quality floors, or terminal thresholds.
...
- **Analysis Plan**: Schema-governed, post-calibration, pre-cohort frozen numeric rules that bind their source calibration protocol and calibration-completion artifact and define floors, workload strata, p95 raw-resource and p95-duration guardrails, clustered paired non-inferiority, Pareto comparison, margins, sample size and assumptions, power, alpha, multiplicity, racing/futility, reruns, attrition, campaign budgets, cache-state isolation, terminal policy, estimand inclusion, and inconclusive outcomes.
...
- **SC-017**: Alias re-pointing is detected from observed-versus-resolved model identity, recorded as bounded platform behavior, and never reported as SpecKit Pro fallback.
- **SC-018**: 100% of role-eligible models have a sanitized ordered effort-ladder record containing their actual default and every ordinary effort advertised by the pinned refreshed Codex catalog; Ultra is never admitted as an ordinary effort.
- **SC-019**: The full default suite passes with zero live calls, a clean payload boundary, execution within its declared wall-clock budget, and frozen-bundle replay p95 under ten seconds on a normal developer checkout.
- **SC-020**: 100% of scored attempts bind and conform to a pre-execution environment contract covering client range, parent model/effort, controlled overrides, authentication mode, and `ultra_state=off`; no harness path mutates Ultra.
- **SC-021**: 100% of accepted score bundles bind a transcript-observed namespaced agent spawn and run-observed effective model, with zero configuration-inferred substitutes.
- **SC-022**: 100% of assigned pairs bind a workload stratum before execution from a non-empty closed pre-execution basis with `derived_from_realized_outcomes=false`; zero frozen strata are redrawn after eligible outcomes.
- **SC-023**: 100% of p95 guardrails contain the full declaration required by FR-053, every stratum carries its own sample size and minimum unique-task count, and below-floor strata return inconclusive.
- **SC-024**: 100% of pairs contributing to resource comparison record observed per-arm cache-root evidence with `status=observed_disjoint`; shared or unobserved roots contribute zero resource comparisons.
- **SC-025**: Racing and futility declarations enumerate every interim look, information fraction, stopping boundary, error control, futility bindingness, and complete-pair stop scope; zero post-outcome look changes are accepted.
