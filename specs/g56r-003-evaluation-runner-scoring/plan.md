...
| VI. KISS/YAGNI | Pass | One materializer, thin evaluation adapter, six bounded contract families, and no cross-vendor or future-policy abstraction. |
...
| Analysis plan | `analysis-plan.schema.json` and frozen analysis-plan data | experiment policy, cohort specs, and replay | calibration reports and draft calculations |
...
- expose one explicit local budgeted calibration command with attempt,
  ...
  candidate-count, and confirmation-entry ceilings, then freeze the
  schema-governed numeric analysis plan before later cohort outcomes;
