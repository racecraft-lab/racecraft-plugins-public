...
**Performance goal**: deterministic validation of a frozen replay bundle in
under 10 seconds on a normal developer checkout; live campaign performance is
recorded, not optimized by this feature  
...
| VI. KISS/YAGNI | Pass | One materializer, thin evaluation adapter, five bounded contract families, and no cross-vendor or future-policy abstraction. |
...
| Experiment policy and decision | `experiment-policy.schema.json`, `analysis-decision.schema.json`, frozen plan data | cohort specs and replay | reports are projections of immutable bundles |
...
comparison assignment
  --comparison_set_id + experiment_policy_id + analysis_plan_id-->
...
- bind candidate/comparator pairs before execution with immutable policy,
  snapshot, materialization, fixture, task, partition, and analysis-plan IDs;
...
- expose one explicit local budgeted calibration command and freeze the
  numeric analysis plan before later cohort outcomes;
...
- calibration report proves `qualification_eligible=false`.
...
1. Validate all bindings, partition membership, treatment eligibility,
   deterministic hard gates, score provenance, and completeness.
...
3. Evaluate the task-paired, role-cluster-adjusted non-inferiority confidence
   bound against the frozen margin and multiplicity rule.
...
Files: 5, Total Files: 15, Primary Surface: harness/adapter. The accepted
