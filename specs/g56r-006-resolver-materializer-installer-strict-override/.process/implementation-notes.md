...
### Post: Review Remediation — Structured Secondary-Failure Packaging

**Deviations/Edge cases/Surprises:** The next exact-head review found that real `ENOTEMPTY` quarantine teardown, cleanup-name reappearance after successful restore, and secondary cleanup/read failures in write/removal packaging could still omit evidence or replace a structured conflict. Six deterministic cases established RED at 169/175 with three failures and three errors. The remediation reports unclassified non-empty quarantine directories as unknown, reclassifies both restore names, and folds secondary failures into `CodexAgentNoClobberConflict` evidence without masking the primary conflict. GREEN and REFACTOR passed 175/175; materialization remained 11/11 and the canonical materializer 17/17.
...
### Post: PR Review Remediation — Static Materialization Compatibility

**Deviations/Edge cases/Surprises:** Live PR review found that `candidate_route=None` required an omitted effort and rewrote source bytes, breaking legacy static qualification for `autopilot-fast-helper`; it also flagged two registry imports as unused. Two focused regressions established RED at 11/12 and 12/13. The remediation preserves source bytes and records an empty omitted effort only when no route is supplied, while explicit routes still require and insert a non-empty effort. The broader installer suite then proved the registry constants are intentional tested exports, so those imports remain for compatibility. GREEN and REFACTOR passed 13/13; the canonical materializer passed 17/17; the installer suite passed 196/196; and the actual Layer 6 qualification publish-materialization command succeeded.
