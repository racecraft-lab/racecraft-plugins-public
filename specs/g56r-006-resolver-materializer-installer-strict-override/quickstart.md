...
- Destructive backup cleanup is followed by target recapture; a cleanup-window race recreates the captured prior bytes and mode under a fresh exclusive backup path, while cleanup evidence omits paths removed by a successful retry.
- Recovery copies and restore operations are descriptor-anchored to the captured destination identity, rollback preserves the original pre-batch state, existing targets move through a native atomic no-replace primitive with collision retry and fail-closed unsupported handling, and incomplete copies use distinct `recovery_copy_failed` evidence.
