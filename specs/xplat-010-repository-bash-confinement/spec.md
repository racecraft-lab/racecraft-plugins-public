...
- **FR-021**: Each feat/fix PR MUST carry a consumer-facing Release note block in its body: exactly one fenced code block whose info-string is `release-note`, containing multiline plain-English prose written for consumers (limited inline markdown allowed; raw HTML stripped by the composer; empty or whitespace-only counts as missing; zero or more-than-one block fails the check). A `.github/pull_request_template.md` seeds the empty block under a `## Release note` heading.
...
- **Release Note Block**: The consumer-facing block each feat/fix PR carries in its body, harvested by the composer at release time. Grammar (per Clarifications Session 3): exactly one fenced code block with info-string `release-note`; multiline plain-English prose; limited inline markdown (emphasis, links, `-` bullets); raw HTML stripped at composition; empty/whitespace-only counts as missing; skip is expressed only by the `release-note/skip` label, never by an in-block sentinel.
