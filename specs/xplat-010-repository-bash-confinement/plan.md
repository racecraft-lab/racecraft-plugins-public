...
(repo-relative only). Shipped-runner byte changes are confined to PRs 2, 7b, 10, and 13, each
carrying the payload rebuild + proof-hash regeneration ritual with release-readiness
evidence regenerated LAST and home-directory sanitization (`<home>`). Because PRs 2, 7b, 10, and 13
...
│   └── speckit-pro-runner.manifest.json  # PRs 2/7b/10/13: sha256 rows; PR 13: plugin_version fix
...
(`speckit-pro/speckit_pro_runner/`) changes only in PRs 2/7b/10/13 and each such change runs the
