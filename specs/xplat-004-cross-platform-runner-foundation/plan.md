...
Build the minimal Python 3.11+ standard-library runner foundation under `speckit-pro/speckit_pro_runner/`, invoked as `<python> -m speckit_pro_runner` from the package context. The implementation preserves the XPLAT-002 JSON stdin/stdout contract and XPLAT-003 Python runtime decision while limiting behavior to runtime-info, preflight, contract smoke fixtures, and source-checkout metadata.
...
No real helper ports, generated payload cutover, active skill/hook switch, install behavior change, or public native-platform support claim is in scope.
...
**Constraints**: JSON request on stdin; one JSON response on stdout; line-delimited JSON diagnostics on stderr; `shell=False` subprocess execution; typed paths; deterministic exit/status mapping; source-checkout `plugin_relative` metadata; no copying into `dist/**`.
...
**Structure Decision**: Use a small package at `speckit-pro/speckit_pro_runner/`. `__main__.py` owns module invocation, `envelope.py` owns JSON request/response and diagnostics, and `runtime.py` owns preflight/runtime-info, typed path rendering, subprocess result capture for fixtures, prerequisite discovery, and source metadata checks.
...
- Accept XPLAT-002 request envelopes on stdin with `schema_version: "1.0"`, `helper_id: "runner"`, `operation: "preflight"` or `"runtime-info"`, and `mode: "read_only"`.
- Return one JSON response on stdout.
- Emit line-delimited JSON diagnostics on stderr.
- Discover Python 3.11+ and `specify` prerequisites, failing closed when unavailable.
- Report `runner_name: "speckit_pro_runner"`, `runner_contract_id: "speckit-pro-runner"`, `selected_runtime_name: "python-stdlib-runner"`, and `source_vs_installed_context: "source_checkout"`.
...
**Primary files**: `contract-fixtures.json`, `speckit-pro-runner.manifest.json`, `speckit-pro-runner.sha256`, and expanded runner tests.

**Behavior**:

- Cover invalid JSON, invalid envelope, unsupported schema version, missing fields, typed path values, paths with spaces, Windows separators, traversal boundary handling, missing prerequisites, subprocess nonzero, subprocess timeout, stderr-only failure, runtime-info, and preflight.
- Keep subprocess execution shell-disabled and fixture-only.
- Store manifest and checksum files under `speckit-pro/speckit_pro_runner/`, using source-checkout `plugin_relative` paths.
- Set metadata `verification_status` to `verified`, `mismatch`, `missing_metadata`, or `not_checked` based on actual checks.
