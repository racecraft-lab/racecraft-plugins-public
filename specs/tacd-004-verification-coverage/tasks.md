...
- [ ] T004 Inventory the active-agent set used by the named-tool guard (US1) and the pointer/resolution checks (US2): list `speckit-pro/agents/*.md` (11 Claude agents) and `speckit-pro/codex-agents/*.toml` (10 Codex agents); record the capability-dependent in-scope subset (`codebase-analyst`, `domain-researcher`, `implement-executor`) and the enumerated out-of-scope exclusion set with a one-line reason each, per spec FR-003 and the spec Assumptions, so "uncovered" cannot be confused with "out of scope".
