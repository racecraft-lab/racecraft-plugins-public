...
* **A fourth surface, discovered during implementation and not anticipated
  here.** The Layer 6 Codex qualification corpus binds a sha256 digest over
  agent source bytes, so editing `codex-agents/implement-executor.toml`
  restales it. Two tracked files carry that binding:
  `tests/speckit-pro/layer6-efficiency/fixtures-codex/corpus-manifest.json` and
  `tests/speckit-pro/layer6-efficiency/fixtures-codex/implement-executor/fixture.json`.
  It is proof data in the same category as the installed-cache proofs, so it is
  excluded from the tracked-file count above for the same reason. Unlike the
  other three it has **no regeneration tooling**, so it was refreshed by hand,
  which the test tree's `AGENTS.md` permits for fixture and proof data with an
  explanation. Flagged for follow-up rather than tooled here.

The first three are regenerated by one idempotent command,
`python3 scripts/refresh-release-artifacts.py`, which does **not** cover the
fourth. Not
