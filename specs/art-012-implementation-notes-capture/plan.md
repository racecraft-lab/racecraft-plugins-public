...
Build and verification use `python3 scripts/build-plugin-payloads.py` and
...
**Reviewability Budget**: Primary surface harness/adapter; 155 projected
...
* `dist/claude/speckit-pro/` and `dist/codex/speckit-pro/` payload copies of the
  five production files, via `python3 scripts/build-plugin-payloads.py`.
...
authoritative figure stays 155, from `estimate-spec-size` with this spec's
...
* Within budget on every dimension: 155 reviewable LOC against a 400 warn line
