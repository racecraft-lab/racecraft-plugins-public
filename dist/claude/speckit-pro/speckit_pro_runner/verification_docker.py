...
def container_options(image_id: str, execution_id: str, *, qualified: bool = False) -> list[str]:
    ...
    return [*options, "--init", f"--name=speckit-verifier-{execution_id}",
...
def container_reasons(info: dict[str, Any], image_id: str, execution_id: str, *, qualified: bool = False) -> list[str]:
    """Compare observed configuration to policy, before AND after execution."""
    container_options(image_id, execution_id, qualified=qualified)
    ...
    if qualified:
        qualified_host = {"Runtime": "runc", "Dns": ["127.0.0.1"], "DnsSearch": ["."],
                          "DnsOptions": ["ndots:0"]}
        reasons.extend(key for key, value in qualified_host.items()
                       if type(host.get(key)) is not type(value) or host[key] != value)
    ...
    if qualified:
        expected["Hostname"] = "speckit-verifier"
...
def build_context(destination: Path, files: dict[str, tuple[int, bytes | None]],
                  reference: str, argv: list[str], qualification_profile: str | None = None) -> dict[str, str]:
    ...
    if not isinstance(reference, str) or IMAGE_REFERENCE.fullmatch(reference) is None:
        raise ValueError("base image reference must be digest-pinned")
    validate_argv(argv)
    if qualification_profile not in (None, "docker-qualified/v2"):
        raise ValueError("unsupported Docker qualification profile")
    request_value = {"argv": argv}
    if qualification_profile is not None:
        request_value["qualification_profile"] = qualification_profile
    request = json.dumps(request_value, ensure_ascii=False, sort_keys=True).encode("utf-8")
