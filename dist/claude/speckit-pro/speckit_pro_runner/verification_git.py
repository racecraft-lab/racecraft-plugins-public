...
QUALIFIED_PROFILE = "git-hermetic-relocated/v2"
...
def read_git_file(path: Path, limit: int) -> bytes:
    ...
    # Each directory descriptor is closed as soon as the next path component is
    # resolved relative to it, and the last one by the ``finally`` below. Closing
    # a parent early is safe because the child descriptor already pins that
    # directory inode; the opened file itself is closed by the ``with`` block.
    ...
    try:
        for part in path.parts[1:-1]:
            ...
            os.close(descriptor)
            descriptor = child
        target = os.open(path.name, os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=descriptor)
    finally:
        os.close(descriptor)
...
def check_git_configuration(body: bytes, *, qualified: bool = False) -> None:
    ...
    for section in parser.sections():
        ...
        for option, raw_value in parser.items(section):
            ...
            forbidden = {"core.worktree", "core.excludesfile", "core.attributesfile", "core.fsmonitor",
                         "core.sshcommand", "core.gitproxy", "core.askpass"}
            if key in forbidden or family in {"filter", "diff"} or (qualified and key == "core.hookspath"):
...
def reject_unqualified_git_dependencies(root: Path, common: Path) -> None:
    """Refuse inputs whose Git behavior depends on uncaptured programs or repositories."""
    hooks = common / "hooks"
    if hooks.exists():
        for path in hooks.iterdir():
            if path.is_symlink() or (path.name.endswith(".sample") and not path.is_file()) or not path.name.endswith(".sample"):
                raise ValueError("unsupported active Git hook dependency")
    if (root / ".gitmodules").exists() or (root / ".gitmodules").is_symlink():
        raise ValueError("unsupported nested Git repository or submodule")
...
def capture_git_metadata(root: Path, settings: Any, *, qualified: bool = False) -> tuple[dict[str, tuple[int, bytes | None]], dict[str, Any]]:
    """Capture bounded metadata; v2 additionally closes hermetic Git dependencies."""
    root = root.resolve(strict=True)
    common, private, controls = git_directories(root, settings)
    if qualified:
        reject_unqualified_git_dependencies(root, common)
    ...
    for base, names in groups:
        for path in metadata_paths(base, names):
            ...
            if configuration:
                ...
                check_git_configuration(body, qualified=qualified)
            mode = stat.S_IMODE(info.st_mode)
            files[name] = (mode, body)
            sources[name] = {"source": str(path), "mode": mode, "sha256": sha(body) if body is not None else None}
            if len(files) > MAX_FILES:
                raise ValueError("Git input closure exceeds snapshot entry limit")
    binding = {"profile": QUALIFIED_PROFILE if qualified else "git-readonly-metadata/v1", "qualified": qualified,
               ...
               "limitations": [] if qualified else list(LIMITATIONS)}
