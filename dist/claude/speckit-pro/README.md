...
1. Copy `dist/codex/speckit-pro/` to `~/.codex/plugins/speckit-pro`
...
When updating the plugin: refresh the generated Codex payload directory the marketplace points to, rerun the install skill if any `codex-agents/*.toml` templates changed, and restart Codex so the install cache and custom-agent registry refresh. Do not install the mixed authoring source tree directly.
