...
- **Inside `/speckit-pro:speckit-scaffold-spec SPEC-NNN`** — automatic and mandatory; grills against the roadmap's scope description for the requested spec, produces `SPEC-NNN-design-concept.md`, and uses it to enrich the workflow file's phase prompts.

Grill-me is strictly human-in-the-loop. It cannot be invoked autonomously, and it never runs from inside `/speckit-pro:speckit-autopilot` — autopilot's clarification mechanism is `/speckit-clarify` plus multi-agent consensus, which is a deliberately different system.
