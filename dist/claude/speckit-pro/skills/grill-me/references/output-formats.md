...
when an agent actually performed the use.
When a present bundle returns no selected concepts, keep both arrays empty but
record that bundle's actual snapshot instead of the empty-bundle snapshot.
...
- **Feed into the technical roadmap.** Run
  `/speckit-pro:speckit-prd --roadmap-only <existing-prd-path>` and reference
  this doc when the reviewed PRD incorporates its decisions.
