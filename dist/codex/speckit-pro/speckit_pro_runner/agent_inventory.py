...
CATEGORIES = frozenset({"shared", "sweep_security", "claude_only", "optional_helper"})
...
def _validate_role_shape(role: dict[str, Any]) -> None:
    ...
    elif category == "claude_only":
        if (claude["implementation"], claude["install_status"]) != ("plugin_agent", "required"):
            raise AgentInventoryError(f"claude-only role {name} must be a required Claude plugin agent")
        if (codex["implementation"], codex["install_status"]) != ("none", "not_installed"):
            raise AgentInventoryError(f"claude-only role {name} must be absent on Codex")
