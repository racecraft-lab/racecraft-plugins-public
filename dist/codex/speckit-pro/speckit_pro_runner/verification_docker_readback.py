...
class SnapshotReadback:
    ...
    def verify(self, client: DockerClient, cid: str, before: dict[str, Any], deadline: float) -> dict[str, Any]:
        return _container_snapshot(client, cid, before, deadline, self.archive, self.snapshot, "created")

    def verify_after(self, client: DockerClient, cid: str, after: dict[str, Any], deadline: float) -> dict[str, Any]:
        return _container_snapshot(client, cid, after, deadline, self.archive.with_name("input-readback-after.tar"),
                                   self.snapshot, "exited")


def _container_snapshot(client: DockerClient, cid: str, info: dict[str, Any], deadline: float, archive: Path,
                        snapshot: dict[str, tuple[int, bytes | None]], status: str) -> dict[str, Any]:
    state = info.get("State")
    if (not isinstance(state, dict) or state.get("Status") != status or state.get("Running") is not False
            or type(state.get("Pid")) is not int or state["Pid"] != 0):
        description = "never-started" if status == "created" else "stopped"
        raise ValueError(f"input readback requires a {description} container")
    remaining = deadline - time.monotonic()
    if remaining <= 0:
        raise ValueError("verification deadline exhausted before input readback")
    copied = client.call(["cp", f"{cid}:/inputs/.", "-"], min(remaining, 120), archive=archive)
    readback = verify_snapshot_archive(archive, snapshot)
