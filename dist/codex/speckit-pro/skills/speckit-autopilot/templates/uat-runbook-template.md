<!--
  ...
  Rendered by generate-uat-skeleton. The eight section headers below are
  emitted in this exact order on every run; the helper substitutes each {{...}} token
  with deterministic content parsed from spec.md. Do not reorder the sections — the
  FR Coverage Matrix and the `## UAT Runbook` embed depend on this order.
-->
# UAT Runbook: {{SPEC_ID}}
