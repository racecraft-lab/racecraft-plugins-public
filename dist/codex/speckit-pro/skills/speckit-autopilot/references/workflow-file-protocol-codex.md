...
the Codex-specific mirror of `workflow-file-protocol.md` — same protocol, Codex-specific commit primitives (parent session direct `apply_patch`).
...
reference](workflow-file-protocol.md).
...
The guard fails with one of three messages:

- `autopilot state workflow_file is not a normalized repository-relative path`:
  the value is machine-written, so the state is untrustworthy, not the workflow.
- `workflow file is outside the authorized repository`: the supplied workflow
  resolves outside the repository that holds the state, possibly through a
  symlink.
- `supplied workflow does not match autopilot state workflow_file authority`:
  the run is resuming the wrong specification. Both compared paths follow the
  message: re-point the run, or reclaim the state slot and let the next
  invocation rewrite it.

A state with no `workflow_file` key asserts no authority, and the check skips.
The comparison follows symlinks and is byte-exact, so keep workflow files and the
state that names them as real paths inside one repository. The branch order, the
reasons for each branch, and how to tell a skip from a satisfied comparison live
in the module docstring and `_workflow_authority_errors` of
